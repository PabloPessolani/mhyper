...
#define NOT_PROC	0x0400  /*  MASK to filter a proc. WARNING!!! NR_PROC must be less than 0x0400 */
...
#define NR_DFT_DRIVERS	22

#define FLOPPY_PROC_NR 	13
#define IS_PROC_NR 		23
#define ATWINI_PROC_NR 	16


#define  DISABLED_PROC_TYPE		0x0000	/* the process will never run in VMx					*/
#define  REAL_PROC_TYPE			0x0001	/* the process will run in the VMx without changes 			*/
#define  VIRTUAL_PROC_TYPE		0x0002	/* the process will never run but its slot is used to receive messages  */
#define  PROMISCUOUS_PROC_TYPE	0x0004	/* the process is prepared to receive requests from ANY VM	 	*/
#define  MH_TYPE(x)				( x & 0x000F)

#define  BOOT_LOADED			0x0000	/* the process is loaded at VM boot time	*/
#define  EXEC_LOADED			0x0010	/* the process is loaded by EXEC  			*/
#define  MH_LOADED(x)			( x  & 0x0010)

#define  DONOT_NTFY				0x0000	/* the process promiscousus process will NOT be notified of a new VM startup	*/
#define  BOOT_NTFY				0x0020	/* the process promiscousus process will  be notified of a new VM startup	*/
#define  EXEC_NTFY				0x0040	/* the process promiscousus process will  be notified of a new VM startup	*/
...
#endif /*  MHYPER */
#define NR_METRICS  1024
#define MAXTOKENS		256	/* maximum number of tokens to allocate for all VMs	*/
#define TICKS_TO_SWITCH		8	/* continuous ticks before trying to switch VM		*/
...
#define VMM_PROC_NR	  7    	/* Virtual Machine Manager server */
...
#ifdef MHYPER
#define NOTIFY_VMID			m2_i2
#define NOTIFY_COUNT		m2_i3
#define NOTIFY_VMBITMAP		m2_p1 
#endif /*  MHYPER */
...
#ifdef MHYPER
#define SYS_GETVMID    (KERNEL_CALL + 31)	/* sys_getvmid() */
#define SYS_VMMREQ     (KERNEL_CALL + 32)	/* sys_vmmreq() */
#define SYS_VCOPYVM    	(KERNEL_CALL + 33)	/* sys_vcopyvm() */
#define SYS_UMAPVM    	(KERNEL_CALL + 34)	/* sys_umapvm() */
#define SYS_KILLVM      (KERNEL_CALL + 35)	/* sys_killvm() */
...
/* Field names for _VIRCOPY_VM (ONLY DATA) */
#ifdef MHYPER
#define CP_SRC_VMID 	m5_c1 /* source VMID 	  */
#define CP_DST_VMID 	m5_c2 /* destination  VMID */
#endif /*  MHYPER */
...
#   define SET_IS_VM      17	/* Set IS_active_vm in kernel */
#   define GET_PRIV_VM 	  18	/* get process privileges */
...
#define I_VAL_VMID     m7_i2	/* VMID */
#define I_VAL_PRIV     m7_i4	/* PRIVILEGE INDEX (ID) */
...
#define SIG_VMID 	   m2_i3	/* VMID of the process to signal */
...
#define DS_DELETE	(DS_RQ_BASE + 3)	/* Delete published information */

#  define DS_KEY		m2_i1		/* key for the information */
#  define DS_FLAGS		m2_i2		/* flags provided by caller */
#  define DS_ENDPOINT		m2_i3		/* flags provided by caller */
#  define DS_AUTH		m2_p1		/* authorization of caller */
#  define DS_PTR		m2_p1		/* authorization of caller */
#  define DS_VAL_L1		m2_l1		/* first long data value */
#  define DS_VAL_L2		m2_l2		/* second long data value */


/*===========================================================================*
 *                Messages for the Virtual Machine Manager Server (VMM) 	     *
 *===========================================================================*/

#define VMM_RQ_BASE		0x900

#define VMM_NCALLS		13

#define	VMM_INFO		(VMM_RQ_BASE + 0)
#define	VMM_LOAD		(VMM_RQ_BASE + 1)
#define	VMM_START		(VMM_RQ_BASE + 2)
#define	VMM_STOP		(VMM_RQ_BASE + 3)
#define	VMM_KILL		(VMM_RQ_BASE + 4)
#define	VMM_SUSPEND		(VMM_RQ_BASE + 5)
#define	VMM_RESUME		(VMM_RQ_BASE + 6)
#define	VMM_SAVE		(VMM_RQ_BASE + 7)
#define	VMM_RESTORE		(VMM_RQ_BASE + 8)
#define	VMM_TABDUMP		(VMM_RQ_BASE + 9)
#define	VMM_VMDUMP		(VMM_RQ_BASE + 10)
#define	VMM_WAITVM		(VMM_RQ_BASE + 11)
#define	VMM_PROC		(VMM_RQ_BASE + 12)

#define VMM_CMD			m1_i1		
#define VMM_I1			m1_i1		
#define VMM_I2			m1_i2		
#define VMM_I3			m1_i3	
#define VMM_P1			m1_p1
