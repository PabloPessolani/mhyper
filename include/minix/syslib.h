...
#define sys_setISvm(vmid)	sys_getinfo(SET_IS_VM, 0,vmid,0,0)
...
#ifdef MHYPER
_PROTOTYPE( int sys_getvmid, 	(void));
_PROTOTYPE( int sys_mhyper, 	(void));

_PROTOTYPE( int sys_vcopyvm , (int src_proc, int src_vmid, vir_bytes src_vir, 
	int dst_proc, int dst_vmid, vir_bytes dst_vir, phys_bytes bytes));
	
_PROTOTYPE(int sys_umapvm, (int vmid, int proc_nr, int seg, vir_bytes vir_addr,
	 vir_bytes bytes, phys_bytes *phys_addr));
	 
_PROTOTYPE(int sys_killvm, (int proc, int sig, int vmid) );
