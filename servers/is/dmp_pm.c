...
PUBLIC void mproc_dmp()
{
  ...
  static int prev_i = 0;
	...
	int rcode;
if(mhyper) {
	printf("Process manager (PM) process table dump for VM%d\n", active_vm);
 	if( active_vm != VM0) {
		rcode = getsysinfo_vm(PM_PROC_NR, SI_PROC_TAB, mproc, active_vm);
		_mhdebug("IS getsysinfo_vm PM_PROC_NR",rcode);
	}else{
		rcode = getsysinfo(PM_PROC_NR, SI_PROC_TAB, mproc);
	}
	if(rcode != OK)
		_mhdebug("PM getsysinfo ",rcode);
}else{
...
PUBLIC void sigaction_dmp()
{
  ...
  clock_t uptime;
	...
	int rcode;
if(mhyper) {
	printf("Process manager (PM) signal action dump for VM%d\n", active_vm);
 	if( active_vm != VM0) {
		rcode = getsysinfo_vm(PM_PROC_NR, SI_PROC_TAB, mproc, active_vm);
		_mhdebug("IS getsysinfo_vm PM_PROC_NR",rcode);
	}else{
...
if(mhyper) {
 	if( active_vm != VM0) {
		rcode = getsysinfo_vm(PM_PROC_NR, SI_MEM_ALLOC, &pmi, active_vm);
		_mhdebug("IS getsysinfo_vm PM_PROC_NR",rcode);
	}else{
		rcode = getsysinfo(PM_PROC_NR, SI_MEM_ALLOC, &pmi);
	}
	if(rcode != OK)
		_mhdebug("PM getsysinfo ",rcode);
		printf("Available memory stats.(VM%d)\n", active_vm);
}else{
