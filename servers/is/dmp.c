...
message *m;					/* notification message */
{
  ...
  /* Now check which keys were pressed: F1-F12, SF1-SF12. */
	for(h=0; h < NHOOKS; h++){
		if(!mhyper && ((hooks[h].key == F8) || (hooks[h].key == F12))) 
			continue;
...
#ifdef MHYPER
if(mhyper)
  printf("Function key mappings for debug dumps in IS server. (VM%d)\n",active_vm);
else
#else /*  MHYPER */
