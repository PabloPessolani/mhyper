...
if(mhyper) {
 	if( active_vm != VM0) {
		getsysinfo_vm(FS_PROC_NR, SI_PROC_TAB, fproc, active_vm);
	} else {
		getsysinfo(FS_PROC_NR, SI_PROC_TAB, fproc);
	}
  	printf("File System (FS) process table dump. (VM%d)\n", active_vm);
}else{
	getsysinfo(FS_PROC_NR, SI_PROC_TAB, fproc);
...
if(mhyper) {
 	if( active_vm != VM0) {
		getsysinfo_vm(FS_PROC_NR, SI_DMAP_TAB, dmap, active_vm);
	} else {
		getsysinfo(FS_PROC_NR, SI_DMAP_TAB, dmap);
	}
  	printf("File System (FS) device <-> driver mappings (VM%d)\n", active_vm);
} else {
    getsysinfo(FS_PROC_NR, SI_DMAP_TAB, dmap);
