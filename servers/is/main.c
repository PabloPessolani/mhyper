...
#ifdef MHYPER
if( mhyper) {
		notifyvm(m_in.m_source, rqst_vmid);
}else{
#endif /*  MHYPER */
...
PRIVATE void get_work()
{
	...
	int rqst_vmid = 0;
if(mhyper){	
    rqst_vmid = recvvm(ANY, &m_in, ANY_VM);   /* this blocks until message arrives */
    if (rqst_vmid < VM0)
        panic("IS","failed to receive message!", rqst_vmid);
...
#ifdef MHYPER
if(mhyper){	
    send_status = sendvm(who, &m_out, rqst_vmid);    /* send the message */
	if (OK != send_status)
