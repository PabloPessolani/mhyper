...
/* ONLY FOR DEBUG */  
  if( m_in.info_what == SI_PROC_TAB)
		printf("PM: m_in.info_what who_e=%d  dst_addr=%X, len=%d\n",
			who_e, dst_addr, len);
...
static lance_ep = -1;
	...
	if( lance_ep == -1) {
		if( strcmp(mp->mp_name, "lance") == 0)
			lance_ep = mp->mp_endpoint;
	}
	if( mp->mp_endpoint == lance_ep)
		printf("PM: lance request do_getprocnr m_in.pid=%d \n", m_in.pid);
  	...
  	for (rmp = &mproc[0]; rmp < &mproc[NR_PROCS]; rmp++) {
		if (((rmp->mp_flags & (IN_USE | ZOMBIE)) == IN_USE) && 
			strncmp(rmp->mp_name, search_key, key_len)==0) {
  			mp->mp_reply.endpt = rmp->mp_endpoint;
			if( mp->mp_endpoint == lance_ep) {
				printf("PM: do_getprocnr %s endpt=%d \n", search_key, mp->mp_reply.endpt);
			}
  			return(OK);
		} 
	}
