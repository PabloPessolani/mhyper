...
#ifdef MHYPER
  u32_t	mp_vmsize;			/* VM size  */
#endif /*  MHYPER */

} mproc[NR_PROCS];

#ifdef MHYPER
#define MPROC_FORMAT "%s:%d pm_vmid=%d name=%s pid=%d endpoint=%d flags=%X wpid=%d\n"
#define MPROC_FIELDS(p)  __FILE__ ,__LINE__, pm_vmid, p->mp_name, p->mp_pid , p->mp_endpoint, p->mp_flags , p->mp_wpid
#endif /*  MHYPER */
...
#define VM_PROC		   0x8000 	/* This process can not be killed */
