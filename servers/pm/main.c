...
FORWARD _PROTOTYPE( int patch_init, (void));
...
/* Main routine of the process manager. */
  ...
  /* This is PM's main loop-  get work and do it, forever and forever. */
  	pm_vmid = _mhdebug("PM: vmid=", pm_vmid);
	
/*	
	if( pm_vmid != VM0) {
		_mhdebug("pseudo fork of init vmid",pm_vmid);
		result = patch_init();
		if(result) {
			_mhdebug("patch_init result",result);
			return(result);
		}
	}
*/	
...
/*===========================================================================*
 *				patch_init										*
 * Pseudo fork of process init 										* 
 *===========================================================================*/
PRIVATE int patch_init(void)
{
  ...
  static struct mproc mp_parent;	/* temporal copy of init descriptor */
  
    rmc = &mproc[INIT_PROC_NR];
	rmp = &mp_parent;
	*rmp= *rmc;				/* copy the descriptor´s content */
	...
	_mhdebug("PM init prog_clicks",prog_clicks);
	...
	_mhdebug("PM init child_abs",child_abs);
	_mhdebug("PM init parent_abs",parent_abs);
	...
	*/
	...
	printf("patch_init: %s T=%X/%X/%d D=%X/%X/%d S=%X/%X/%d\n", rmc->mp_name, 
				rmc->mp_seg[T].mem_vir, rmc->mp_seg[T].mem_phys, rmc->mp_seg[T].mem_len,
				rmc->mp_seg[D].mem_vir, rmc->mp_seg[D].mem_phys, rmc->mp_seg[D].mem_len,
				rmc->mp_seg[S].mem_vir, rmc->mp_seg[S].mem_phys, rmc->mp_seg[S].mem_len);
				
	/**********************************************************************/
	/* AQUI HABRIA QUE LIBERAR LA MEMORIA DEL INIT ORIGINAL 		*/
	/* SIMILAR A LO QUE HACE pm_exit								*/
	/**********************************************************************/

  return(OK);		 	/* child's pid */
}
...
PRIVATE void get_work()
{
	...
	if( pm_vmid != VM0)
		_mhdebug("get_work vmid",pm_vmid);
#endif /*  MHYPER */
...
/* Wait for the next message and extract useful information from it. */
...
#ifdef MHYPER
	if( pm_vmid != VM0){
		_mhdebug("PM get_work who_e",who_e);
		_mhdebug("PM get_work call_nr",call_nr);
	}
#endif /*  MHYPER */
  ...
  mp = &mproc[who_p < 0 ? PM_PROC_NR : who_p];
	...
	s=sys_getproc(&p, who_e);
	if( s == OK) {
		if( p.p_rts_flags == SLOT_FREE) 
			panic(__FILE__, "PM endpoint number out of sync with source",
				mp->mp_endpoint);
		mp->mp_endpoint = p.p_endpoint;
		mp->mp_flags = (IN_USE|SEPARATE|PRIV_PROC|DONT_SWAP); 
	}else {
		panic(__FILE__, "PM endpoint number out of sync with source",
			mp->mp_endpoint);
	}
...
*/
  int s;
#ifdef MHYPER
  static struct boot_image image[PROCESS_MAX+NR_TASKS];
#else /*  MHYPER */
  ...
  if ((s=sys_getkinfo(&kinfo)) != OK)
	...
	if( pm_vmid != VM0) {
 		...
 		_mhdebug("kmem_base",kinfo.kmem_base);		/* kernel memory layout (/dev/kmem) */
  		...
  		_mhdebug("nr_bprocs",kinfo.nr_bprocs);		/* number of bootprocess */
  ...
  for (ip = &image[0]; ip < &image[kinfo.nr_bprocs]; ip++) {		
#else /*  MHYPER */
  printf("Building process table:");		/* show what's happening */
  for (ip = &image[0]; ip < &image[NR_BOOT_PROCS]; ip++) {		
 	...
 	if (ip->proc_nr >= 0) {			/* task have negative nrs */
		...
		if( pm_vmid == VM0){
  			...
  			_mhdebug(ip->proc_name,ip->proc_nr);	/* display process name */
	  		printf("pm_init: %s T=%X/%X/%d D=%X/%X/%d S=%X/%X/%d\n", rmp->mp_name, 
				rmp->mp_seg[T].mem_vir, rmp->mp_seg[T].mem_phys, rmp->mp_seg[T].mem_len,
				rmp->mp_seg[D].mem_vir, rmp->mp_seg[D].mem_phys, rmp->mp_seg[D].mem_len,
				rmp->mp_seg[S].mem_vir, rmp->mp_seg[S].mem_phys, rmp->mp_seg[S].mem_len);	
  ...
  /* Tell FS that no more system processes follow and synchronize. */
	...
	if( pm_vmid == VM0){
		...
		mess.PR_ENDPT = NONE;
		if (sendrec(FS_PROC_NR, &mess) != OK || mess.m_type != OK)
			_mhdebug("can't sync up with FS", NO_NUM);
		else
			_mhdebug("sync up with FS", 0);
	}
#else /*  MHYPER */
  mess.PR_ENDPT = NONE;
  if (sendrec(FS_PROC_NR, &mess) != OK || mess.m_type != OK)
...
#ifdef MHYPER
	if( pm_vmid != VM0){
  		_mhdebug("Physical memory",0);
	  	mem_init(mem_chunks, &free_clicks);
 	 	total_clicks = minix_clicks + free_clicks;
  		_mhdebug(" total KB", click_to_round_k(total_clicks));
  		_mhdebug(" system KB", click_to_round_k(minix_clicks));
  		_mhdebug(" free KB", click_to_round_k(free_clicks));
	}else{
  		...
  		printf(" free %u KB", click_to_round_k(free_clicks));
	}
#else /* MHYPER */
  printf("Physical memory:");
  mem_init(mem_chunks, &free_clicks);
 ...
 *				get_nice_value				     *
 *===========================================================================*/
PUBLIC int get_nice_value(queue)
 ...
 */
  ...
  for (i = 0; i < NR_MEMS && !done; i++) {
	...
	if (*s != 0) {			/* get fresh data, unless at end */	
		...
		_mhdebug("PM size",size);
	}
	limit = base + size;	
	_mhdebug("PM limit",limit);
	...
	if( pm_vmid != VM0){
    	_mhdebug("mem_chunks base",memp->base);
       	_mhdebug("mem_chunks size",memp->size);
	}
  }
}
...
_mhdebug("do_x86_vm",0);
	...
	high <<= CLICK_SHIFT;
#if MHYPER
	_mhdebug("do_x86_vm: found high", high);
#endif
...
#if MHYPER
	_mhdebug("do_x86_vm: need pages", pages);
	_mhdebug("do_x86_vm: need bytes", bytes);
	_mhdebug("do_x86_vm: need clicks", clicks);
#endif
...
#ifdef MHYPER
	if (i >= NR_MEMS)
		_mhdebug("PM not enough memory", NO_NUM);
#else /*  MHYPER */
	if (i >= NR_MEMS)
...
#if MHYPER
	_mhdebug("do_x86_vm: using clicks", clicks);
#endif
	r= sys_vm_setbuf(base_click << CLICK_SHIFT, clicks << CLICK_SHIFT,
		high);
#ifdef MHYPER
	if (r != 0)
		_mhdebug("do_x86_vm: sys_vm_setbuf", r);
#else /*  MHYPER */
	if (r != 0)
