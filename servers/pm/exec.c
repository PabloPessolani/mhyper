 ...
 */
...
#ifdef MHYPER
    if(pm_vmid == VM0 ) {	
		vmsize = (vir_clicks) m_in.vm_size; /*in CLICKS */
		/* only superuser can create VMs			*/
		if ((vmsize != 0) && (mp->mp_effuid != SUPER_USER))
			MHERROR("Must be root to create a VM. rcode=%d\n",EPERM);
	} else {
		vmsize =  0;
	}
#endif /*  MHYPER */
  ...
  if (r != OK) return(r);	/* file name not in user data segment */
	...
	if(pm_vmid != VM0)
		_mhdebug(name_buf, m_in.exec_len);
  ...
  /* 
    ...
    *     |			|
    *     ------------------  
    *     |	DATA	| size = data_clicks
    *     ------------------ <<<< proc->p_memmap[D].mem_vir;
    *     |	TEXT	| size= proc->p_memmap[T].mem_len
    *     |			|
    *     ------------------ <<<< proc->p_memmap[T].mem_vir; 
    */
  if( vmsize > 0)	{
		...
		tot_clicks = vmsize;
		gap_clicks = tot_clicks - data_clicks - stack_clicks - text_clicks;
		tot_bytes = tot_clicks << CLICK_SHIFT;  
		rmp->mp_vmsize = gap_clicks; /* REAL SIZE of the MEMORY that can be used by the VM */
	}

	if( vmsize > 0)	
		printf("PM:do_exec: vmsize %d, text_bytes %d, data_bytes %d, bss_bytes %d, stk_bytes %d, tot_bytes %d \n",
			vmsize, text_bytes, data_bytes, bss_bytes, stk_bytes, tot_bytes); 	
  ...
  if( vmsize > 0)
		rmp->mp_ino = (ino_t) 0; /* Null inode to avoid code sharing */
  else
