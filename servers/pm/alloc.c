 ...
 */
  ...
  do {
		...
		hp = hole_head;
		if( pm_vmid != VM0) {
		_mhdebug("PM alloc_mem h_base",hp->h_base);
		_mhdebug("PM alloc_mem h_len",hp->h_len);
		}
	...
	if ( pm_vmid != VM0) {
  		for (i=NR_MEMS-1; i>=0; i--) {
  			if (chunks[i].size > 0) {
				...
				_mhdebug("chunk base", chunks[i].base);
				_mhdebug("chunk size", chunks[i].size);
				*free += chunks[i].size;
			}
  		}
	}else {
		printf("\n");
  		for (i=NR_MEMS-1; i>=0; i--) {
  			if (chunks[i].size > 0) {
				free_mem(chunks[i].base, chunks[i].size);
				printf("Chunks[%d] base=%0.8X size=%d\n",i, chunks[i].base, chunks[i].size);
				*free += chunks[i].size;
#if ENABLE_SWAP
