...
/* Read a superblock. */
  ...
  static char sbbuf[_MIN_BLOCK_SIZE];
	...
	if (fs_vmid != VM0)
		_mhdebug("FS: read_super vmid",fs_vmid);
...
if (fs_vmid != VM0)
		_mhdebug("FS: Calculate OK",fs_vmid);
  ...
  if ((sp->s_block_size % V2_INODE_SIZE) != 0 ||
	...
	if (fs_vmid != VM0)
		_mhdebug("FS: block size OK",sp->s_block_size);
  ...
  if (sp->s_imap_blocks < 1 || sp->s_zmap_blocks < 1
	...
	if (fs_vmid != VM0)
		_mhdebug("FS: read_super bad super",fs_vmid);			
