  ...
  while (TRUE) {
	...
	if (call_nr == PROC_EVENT) {
		...
		/* Not supposed to happen. */
			printf("FS%d, warning illegal %d system call by %d\n", fs_vmid, call_nr, who_e);
...
{  
  ...
  register struct fproc *rp;
	...
	if( fs_vmid != VM0)
		_mhdebug("FS: get_work vmid",fs_vmid);
#endif /*  MHYPER */
...
#ifdef MHYPER
	if( fs_vmid != VM0){
		_mhdebug("FS1 get_work who_e",who_e);
		_mhdebug("FS1 get_work call_nr",call_nr);
	}
#endif /*  MHYPER */			
			return;
		}
  ...
  for(;;) {
    ...
    if(who_p < -NR_TASKS || who_p >= NR_PROCS)
	    ...
	    (who_p >= 0 && fproc[who_p].fp_endpoint != who_e)) {
		...
		if( s == OK) {
			if( p.p_rts_flags == SLOT_FREE) {
				...
				fproc[who_p].fp_endpoint = p.p_endpoint;
			}
	 	}
	}
...
#ifdef MHYPER
	if( fs_vmid != VM0){
		_mhdebug("FS2 get_work who_e",who_e);
		_mhdebug("FS2 get_work call_nr",call_nr);
	}
#endif /*  MHYPER */
...
#ifdef MHYPER
	fs_vmid = _mhdebug("FS: vmid=", fs_vmid);
	fs_vmid = _mhdebug("FS: vmid=", fs_vmid);
#endif /*  MHYPER */ 
