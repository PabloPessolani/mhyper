...
#define NR_BOOT_PROCS	nr_bprocs
...
#define RS_PERIODS 	2
		   if (rp->r_alive_tm < rp->r_check_tm) { 
			if (now - rp->r_alive_tm > RS_PERIODS*rp->r_period &&
		      rp->r_pid > 0) { 
#if VERBOSE
                      printf("RS: service %d reported late\n", rp->r_proc_nr_e); 
#endif
				kp_ptr = &k_proc;
				sys_getproc(kp_ptr,rp->r_proc_nr_e);
				printf(PROC_FORMAT, PROC_FIELDS(kp_ptr));
				if( (kp_ptr->p_rts_flags & RECEIVING) 
					&& (kp_ptr->p_getfrom_e != ANY)) {
					rp->r_alive_tm = now;
					kp_endp = kp_ptr->p_endpoint;
					do {
						sys_getproc(kp_ptr,kp_ptr->p_getfrom_e);
						printf(PROC_FORMAT, PROC_FIELDS(kp_ptr));
						if( !(kp_ptr->p_rts_flags & RECEIVING) 
							|| (kp_ptr->p_getfrom_e == ANY)) 
							break;
					} while (kp_endp != kp_ptr->p_endpoint);
					if( kp_endp == kp_ptr->p_endpoint) {
				      panic("RS", "deadlock found", s);
					}
				}else{
...
#if VERBOSE
				_mhdebug("RS notify",rp->r_proc_nr_e);
/*
				if(!strncmp(rp->r_cmd, "lance", 5) ){
                  printf("RS:status request to %d\n", rp->r_proc_nr_e); 
				  	  _mhdebug("RS notify",rp->r_proc_nr_e);
				}
*/				
#endif
...
#ifdef MHYPERXXXX
      s=sys_vircopy_vm(SELF, VM0, src_addr, dst_proc, vmid, dst_addr, len);
	  _mhdebug("RS sys_vircopy_vm s",s);
	  if (s) return(s);
#else /*  MHYPER */
