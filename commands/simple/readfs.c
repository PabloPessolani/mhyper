...
_PROTOTYPE(int dump_bm,(int block, int fd));
_PROTOTYPE(int dump_inodes, (int fd));
 ...
 */
{
  ...
  /* Read the superblock */
  super_b = SUPER_BLOCK_BYTES; /*(off_t) 1 *(off_t) BLOCK_SIZE; */
  ...
  magic = sb.s_magic;
  fprintf(stdout, 
	"SUPER_MAGIC=%X SUPER_REV=%X SUPER_V2=%X SUPER_V2_REV=%X SUPER_V3=%X\n"
	, SUPER_MAGIC, SUPER_REV, SUPER_V2, SUPER_V2_REV, SUPER_V3 );
	
  sbp  = &sb;
  fprintf(stdout,SB1_FORMAT,SB1_FIELDS(sbp)); 
  if (!(sb.s_magic == SUPER_MAGIC) && !(sb.s_magic == SUPER_REV)) {
	fprintf(stdout,SB2_FORMAT,SB2_FIELDS(sbp)); 
  }
  ...
  fprintf(stdout, "superblock inodes_per_block=%d\n", inodes_per_block);
  /* Is it really a MINIX filesystem ? */
  if (magic != SUPER_MAGIC && magic != SUPER_V2 && magic != SUPER_V2_REV && magic !=  SUPER_V3) {
	fprintf(stderr, "%s is not a valid MINIX filesystem\n", special_file);
	return;
  }

  /* Fetch the inode of the root directory */
  	fprintf(stdout,"ROOT_INODE:\n"); 
	if (magic == SUPER_MAGIC || magic == SUPER_REV) { /* VERSION 1 */
		...
		if (get_inode(special, (ino_t) ROOT_INODE, (d1_inode*)&root_inode2) < 0) {
			fprintf(stderr, "cannot get inode of root directory\n");
			return;
		}
		/* Extract (recursively) the root directory */
		dump_dir(special, (d1_inode*)&root_inode2, directory);
	}
	...
	fprintf(stdout, "ZONE BITMAP(%d)\n", sb.s_zmap_blocks);
	for(i = 0; i < sb.s_zmap_blocks; i++)
		dump_bm(2+sb.s_imap_blocks+i, special);
...
#define BYPES_PER_LINE	16
int dump_bm(block, fd)
int fd;
{
static   struct buf bp;
	int lines, i , j;
	unsigned char *bmp, *save_bmp;
	
	lines = sb.s_block_size/BYPES_PER_LINE;
	
	/* Fetch the block */
	if (get_block(fd, block, &bp, B_DATA) == 0) {
		bmp = (unsigned char *)&bp;
		for(i = 0; i < lines; i++) {
			save_bmp = bmp;
			for(j = 0; j < BYPES_PER_LINE; j++) {
				fprintf(stdout,"%02X ", *bmp);
				bmp++;		
			}
			bmp = save_bmp;
			for(j = 0; j < BYPES_PER_LINE; j++) {
...
static   struct buf bp;
	block_t ini_block;
	int nr_blocks, inode_nr, b, i;
	d2_inode *ip;
	
	ini_block = 1 + 1 + sb.s_imap_blocks + sb.s_zmap_blocks;
    nr_blocks = sizeof(d2_inode) * sb.s_ninodes;
	nr_blocks /= sb.s_block_size;

	fprintf(stdout,"first_inode_block=%d number_of_inode_blocks=%d\n", ini_block, nr_blocks);	
	inode_nr = 0;
	for( b= 0; b < nr_blocks ; b++){
		if (get_block(fd, (ini_block+b), &bp, B_DATA) == 0) {
			ip = (d2_inode *) &bp;
			fprintf(stdout,"--inode--- mode link -uid -gid --size--\n");	
			for( i = 0; i < inodes_per_block; i++) {
				if( ip->d2_nlinks > 0)
					fprintf(stdout,"[%8d] %04X %4d %4d %4d %8ld\n", 
						inode_nr, ip->d2_mode,ip->d2_nlinks,ip->d2_uid,ip->d2_gid,ip->d2_size);				
				inode_nr++;
				ip++;
			}
		}
 ...
 */
  ...
  block += ino_block;
	fprintf(stdout,"get_inode: ino_block=%d ino_offset=%d block=%d\n", ino_block,ino_offset,block); 

  /* Fetch the block */
  if (get_block(fd, block, &bp, B_INODE) == 0) {
    if(magic == SUPER_MAGIC || magic == SUPER_REV) { /* VERSION 1*/
		memcpy((void *) ip, (void *) &bp.b_v1_ino[ino_offset], sizeof(d1_inode));
		fprintf(stdout,I1_FORMAT,I1_FIELDS(ip)); 
	}else{
		ip2 = (d2_inode *) ip;
		memcpy((void *) ip2, (void *) &bp.b_v2_ino[ino_offset], sizeof(d2_inode));
		fprintf(stdout,I2_FORMAT,I2_FIELDS(ip2)); 
	}
	return(0);
  }
 ...
 */
{
  ...
  register struct cache_block *cache_p, *cp;
  	...
  	fprintf(stdout, "get_block: block=%d/%X type=%d\n",block,block, type);
  ...
  /* fprintf(stdout, "First find out if block requested is in the cache\n"); */
  ...
  if (cp == (struct cache_block *) 0) {	/* block is not in cache */
	/* fprintf(stdout, "block is not in cache\n"); */
	...
	/* fprintf(stdout, "Fill the buffer with the right block\n"); */
...
/* Read a block from the disk. */
{
  ...
  pos = (off_t) blockno *(off_t) sb.s_block_size;

  fprintf(stdout, "get_rawblock: blockno=%d/%X pos=%d/%X\n", blockno, blockno,pos, pos);

  /* Read the block from the disk */
  if (lseek(special, pos, SEEK_SET) == pos
      && read(special, bufp, sb.s_block_size) == sb.s_block_size)
	return(0);
...
/* Show information about the given file/dir in `mkfs'-format */
{
  ...
  ip2 = (d2_inode *) ip;
	...
	if(magic == SUPER_MAGIC || magic == SUPER_REV) { /* VERSION 1*/
		...
		c1 = special_chars[(ip2->d2_mode >> 13) & 03];
		c2 = ((ip2->d2_mode & ALL_MODES & ~RWX_MODES) == I_SET_UID_BIT) ? 'u' : '-';
		c3 = ((ip2->d2_mode & ALL_MODES & ~RWX_MODES) == I_SET_GID_BIT) ? 'g' : '-';
		...
		printf("%c%c%c%03o %d %d", c1, c2, c3,
			(ip2->d2_mode & RWX_MODES), ip2->d2_uid, ip2->d2_gid);

		switch (ip2->d2_mode & I_TYPE) {
			case I_DIRECTORY:
				printf(" %d", ip2->d2_size);
				break;
			case I_CHAR_SPECIAL:	/* Print major and minor dev numbers */
			printf(" %d %d", (ip2->d2_zone[0] >> MAJOR) & 0377,
				(ip2->d2_zone[0] >> MINOR) & 0377);
				printf(" %d", ip2->d2_size);
				break;
			case I_BLOCK_SPECIAL:	/* Print major and minor dev numbers */
				printf(" %d %d", (ip2->d2_zone[0] >> MAJOR) & 0377,
					(ip2->d2_zone[0] >> MINOR) & 0377);
				/* Also print the number of blocks on the device */
				printf(" %ld", (ip2->d2_size / (off_t) sb.s_block_size));
				printf(" %d", ip2->d2_size);
				break;
			default:			/* Just print the pathname */
				printf(" %d %s", ip2->d2_size,  path);
			break;
		}
