...
/* Record where the kernel and the monitor are. */
#ifdef MHYPER
  vm_hyper[VM0].vm_kinfo.code_base = seg2phys(cs);
  vm_hyper[VM0].vm_kinfo.code_size = (phys_bytes) &etext;	/* size of code segment */
  vm_hyper[VM0].vm_kinfo.data_base = seg2phys(ds);
  vm_hyper[VM0].vm_kinfo.data_size = (phys_bytes) &end;		/* size of data segment */
#else /*  MHYPER */
  ...
  vm_hyper[VM0].vm_kinfo.params_base = seg2phys(mds) + parmoff;
  vm_hyper[VM0].vm_kinfo.params_size = MIN(parmsize,sizeof(params)-2);
#ifdef MHYPER
  phys_copy(vm_hyper[VM0].vm_kinfo.params_base, VIR2PHYS(&vm_hyper[VM0].vm_boot_params), kinfo.params_size);
#else /*  MHYPER */
 ...
 *===========================================================================*/
#ifdef MHYPER
PUBLIC char *get_value(params, name)
