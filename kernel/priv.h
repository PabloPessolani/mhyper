...
struct priv {
  ...
  int s_irq_tab[NR_IRQ];
...
#ifdef MHYPER
	int s_vmid;			/* VMID of this privilege descriptor 							*/
	unsigned long s_vm_bitmap;	/* PROMISCUOUS: bitmap of VMs the process serve		 */
	unsigned long s_dr_bitmap;	/* PROXIES: bitmap of drivers that the proxy  emulates		*/
	...
	proc_nr_t s_nr[NR_VMS];	/* p_nr of a promiscuous process on each VM */
#endif /*  MHYPER */
...
#define PRIV_FORMAT "%s:%d s_proc_nr=%d s_id=%d s_flags=%X s_nr_irq=%X s_vmid=%d s_vm_bitmap=%X s_dr_bitmap=%X\n"
#define PRIV_FIELDS(p)  __FILE__ ,__LINE__, p->s_proc_nr,p->s_id,p->s_flags,p->s_nr_irq, p->s_vmid, p->s_vm_bitmap, p->s_dr_bitmap

#define BEG_PRIV_ADDR 	(&vm_hyper[running_vm].vm_priv[0])
#define END_PRIV_ADDR 	(&vm_hyper[running_vm].vm_priv[NR_SYS_PROCS])

#define priv_addr(i)     (vm_hyper[running_vm].vm_ppriv_addr)[(i)]
