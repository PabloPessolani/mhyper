...
#ifdef MHYPER
#ifdef MHDBG 
#define MHDEBUG if(debug_switch) kprintf
#else  /*  MHDBG */ 
#define MHDEBUG if(0) kprintf
#endif  /*MHDBG */

#define MHERROR(T,X) 	do { 			\
					kprintf("ERROR %s:%u: " T,__FILE__,__LINE__, X);\
					return(X);	\
				}while(0)

#define MHASSERT(C) if(!(C)) {\
						kprintf("ASSERT %s:%u \n", __FILE__,__LINE__);\
						panic("proc.c",__LINE__);\
						}

#define MHRETURN(X)	do { \
					MHDEBUG("RETURN %s:%u: rcode=%d\n",__FILE__,__LINE__, X);\
					return(X);	\
				}while(0)
