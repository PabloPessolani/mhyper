...
#ifdef MHYPER
_PROTOTYPE( struct proc *get_proc_addr, (int proc_nr)		);
_PROTOTYPE( struct proc *proc_addr_vm, (int vmid, int proc_nr)	);
_PROTOTYPE( void next_rvm, (void)					);
_PROTOTYPE( void proc_rvm, (void)					);
_PROTOTYPE( char *ul2a, (u32_t n, unsigned b));
_PROTOTYPE( void lock_set_rvm, (int vmid));
_PROTOTYPE( void unlock_set_rvm, (int vmid));
#endif /*  MHYPER */
...
#endif
#ifdef MHYPER
_PROTOTYPE( int isokendpt_vm, (int vmid, int e, int *p)			);
#endif /*  MHYPER */

#ifdef MHYPER
#include <string.h>
_PROTOTYPE( int raw_notify, (int src_p, int dst_p)			);
#endif /*  MHYPER */
...
#ifdef MHYPER
_PROTOTYPE( char *get_value, (_CONST char *params, _CONST char *name));
#endif /*  MHYPER */
...
_PROTOTYPE( int virtual_copy, (struct vir_addr *src, struct vir_addr *dst, 
				vir_bytes bytes) 			);
#ifdef MHYPER
_PROTOTYPE( int virtual_copy_vm, (struct vir_addr_vm *src, struct vir_addr_vm *dst, 
				vir_bytes bytes) 			);
#endif /*  MHYPER */
