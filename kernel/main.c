...
/* Start the ball rolling. */
...
#ifdef MHYPER
	systask_ptr = (&vm_hyper[VM0].vm_proc[NR_TASKS + SYSTEM]);
	clock_ptr   = (&vm_hyper[VM0].vm_proc[NR_TASKS + CLOCK] );
	hard_ptr   = (&vm_hyper[VM0].vm_proc[NR_TASKS + HARDWARE] );
	idle_ptr    = (&vm_hyper[VM0].vm_proc[NR_TASKS + IDLE] );
	
	for(vmid= VM0; vmid < NR_VMS; vmid++) {
 		running_vm = vmid;		/*running_vm is used by macros!! */
		vm_hyper[vmid].vm_vmid = vmid;
		vm_hyper[vmid].vm_time = 0;
 		for (rp = BEG_PROC_ADDR, i = -NR_TASKS; rp < END_PROC_ADDR; ++rp, ++i) {
 			rp->p_rts_flags = SLOT_FREE;		/* initialize free slot */
 			rp->p_nr = i;				/* proc number from ptr */
 			rp->p_endpoint = _ENDPOINT(0, rp->p_nr); /* generation no. 0 */
 			rp->p_vmid = vmid;			/* proc structure VMID */	
 			rp->p_proxy = VM0;			/* the process is a proxy of this VM */
 			rp->p_recv_vm = ANY_VM;			/* Wait to recv a message from VM */
			(vm_hyper[vmid].vm_pproc_addr + NR_TASKS)[i] = rp;     /* proc ptr from number */
		}

  		for (sp = BEG_PRIV_ADDR, i = 0; sp < END_PRIV_ADDR; ++sp, ++i) {
			sp->s_proc_nr = NONE;			/* initialize as free */
			sp->s_id = i;				/* priv structure index */
			sp->s_vmid = vmid;			/* priv structure VMID */
			sp->s_vm_bitmap = (1 << vmid);
			sp->s_dr_bitmap = 0;
			sp->s_call2proxy = 0;
			(vm_hyper[vmid].vm_ppriv_addr)[i] = sp;	/* priv ptr from number */
   ...
   */
...
#ifdef MHYPER
	memcpy(vm_hyper[VM0].vm_VMM.vmm_img.image, image, NR_BOOT_PROCS * sizeof(struct boot_image) ); 
	vm_hyper[VM0].vm_kinfo.nr_bprocs = NR_BOOT_PROCS;
	vm_hyper[VM0].vm_kinfo.vm_bitmap = (1 << VM0);  
	vm_hyper[VM0].vm_VMM.vmm_img.nr_bprocs = NR_BOOT_PROCS;
	vm_hyper[VM0].vm_flags =  (VM_LOADED | VM_RUNNING);
	vm_hyper[VM0].vm_uptime = 0;
	vm_hyper[VM0].vm_bsize = MAXTOKENS;
	...
	MHDEBUG("do_serial_debug=%d\n",do_serial_debug);
	for(vmid=1; vmid < NR_VMS; vmid++) {
		vm_hyper[vmid].vm_flags = VM_FREE;
		vm_hyper[vmid].vm_start = 0;
		vm_hyper[vmid].vm_end   = 0;
		vm_hyper[vmid].vm_a_out = vm_hyper[VM0].vm_a_out;
		memcpy(&vm_hyper[vmid].vm_kinfo		, &vm_hyper[VM0].vm_kinfo 	, sizeof(kinfo_t)); 
		memcpy(&vm_hyper[vmid].vm_machine	,&vm_hyper[VM0].vm_machine	,sizeof(machine_t));
		memcpy(&vm_hyper[vmid].vm_kmess		,&vm_hyper[VM0].vm_kmess	,sizeof(struct kmessages));
		memcpy(&vm_hyper[vmid].vm_krandom	,&vm_hyper[VM0].vm_krandom	,sizeof(struct randomness));
		memcpy(&vm_hyper[vmid].vm_kloadinfo	,&vm_hyper[VM0].vm_kloadinfo,sizeof(struct loadinfo));
		memcpy(&vm_hyper[vmid].vm_boot_params,&vm_hyper[VM0].vm_boot_params, (SECTOR_SIZE + 1));
		vm_hyper[vmid].vm_bsize  = 0;
		vm_hyper[vmid].vm_tokens = 0;
		for( i= 0; i < NR_SYS_PROCS; i++){
			if ( i  < NR_DFT_DRIVERS)
				vm_hyper[vmid].vm_driver[i] = dflt_driver[i]; 
			else{
				strcpy(vm_hyper[vmid].vm_driver[i].drv_name,"NO_DRIVER"); 
				vm_hyper[vmid].vm_driver[i].drv_type = DISABLED_PROC_TYPE;
			}
		}
	}
#endif /*  MHYPER */
...
PRIVATE void announce(void)
{
  ...
  kprintf("\nMINIX %s.%s."
      ...
      "VM%d - Hypervisor %s developed for UTN Project PID-1766\n" 
	" - Facultad Regional Santa Fe - Argentina\n",
      OS_RELEASE, OS_VERSION, running_vm, MHYPER_DATE);
#else  /*  MHYPER */
  kprintf("\nMINIX %s.%s."
      "Copyright 2006, Vrije Universiteit, Amsterdam, The Netherlands\n",
