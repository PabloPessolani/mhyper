...
#ifdef MHYPER
  int vmid;					/* VMID of the process that set the hook*/
#endif /*  MHYPER */
...
#ifdef MHYPER
#define HOOK_FORMAT 		"irq=%d id=%d proc_nr_e=%d notify_id=%d policy_t=%X vmid=%d\n"
#define HOOK_FIELDS(hp) 	hp->irq,hp->id, hp->proc_nr_e, hp->notify_id, hp->policy, hp->vmid
#endif /*  MHYPER */
