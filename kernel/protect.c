 ...
 */
  ...
  * (u32_t *) dtp->base = vir2phys(idt);
  ...
  init_codeseg(&gdt[CS_INDEX],
  	 vm_hyper[VM0].vm_kinfo.code_base, vm_hyper[VM0].vm_kinfo.code_size, INTR_PRIVILEGE);
  init_dataseg(&gdt[DS_INDEX],
  	 vm_hyper[VM0].vm_kinfo.data_base, vm_hyper[VM0].vm_kinfo.data_size, INTR_PRIVILEGE);
#else /*  MHYPER */
  init_codeseg(&gdt[CS_INDEX],
