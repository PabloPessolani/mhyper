 ...
 */
	...
	int saved_vm, clock_vm;
	int proc_nr;

	lock(0, "cause_alarm");
	saved_vm = running_vm;
	clock_vm = running_vm;
 	rp = (struct proc * ) tmr_arg(tp)->ta_ptr;
	running_vm= rp->p_vmid;
	proc_nr = rp->p_nr;
	clock_ptr->p_vmid = running_vm;
	systask_ptr->p_vmid = running_vm;
	raw_notify(CLOCK, proc_nr);
	running_vm = saved_vm;
	clock_ptr->p_vmid = clock_vm;
