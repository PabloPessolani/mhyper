...
if(who_e == 35555){				
		kprintf(
		"do_irqctl: IRQ proc %d, IRQ %d\n",	m_ptr->m_source, irq_vec);
		kprintf(PROC_FORMAT, PROC_FIELDS(rp));					
		kprintf(MSG5_FORMAT, MSG5_FIELDS(m_ptr));					
}			
#endif /* AT_WINI_DBG */	  
      privp= priv(rp);
      if (!privp)
	...
	*/
	...
	if (notify_id > CHAR_BIT * sizeof(irq_id_t) - 1) return(EINVAL);
	  ...
	  hook_ptr->vmid = rp->p_vmid;
      put_irq_handler(hook_ptr, irq_vec, generic_handler);

#ifdef AT_WINI_DBG
if(who_e == 35555){				
		kprintf(HOOK_FORMAT, HOOK_FIELDS(hook_ptr));					
}
#endif /* AT_WINI_DBG */	  
 ...
 */
   ...
   */
	...
	if(!isokendpt_vm(hook->vmid, hook->proc_nr_e, &proc)) {
		kprintf("generic_handler:"HOOK_FORMAT, HOOK_FIELDS(hook));					
		hook->proc_nr_e = NONE;
		return 0;
	}	
...
#ifdef MHYPER
/*
        lock(9,"generic_handler");
        saved_vm 	= running_vm;
        running_vm	= hook->vmid;
