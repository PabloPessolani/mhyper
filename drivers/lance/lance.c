...
#endif
  	lance_vmid = _mhdebug("LANCE: vmid=", lance_vmid);
...
/*printf( "." );*/
      ...
      switch (m.m_type){
      case DEV_PING:
		  	_mhdebug("LANCE: DEV_PING", m.m_source);
			notify(m.m_source);		
			continue;
...
message *mp;
{
  ...
  message reply_mess;
  	...
  	_mhdebug("do_init: source=", mp->m_source);

	pci_init();

  port = mp->DL_PORT;
  	_mhdebug("do_init: port=", port);
...
int may_block;
{
  ...
  reply.DL_CLCK = 0;
	...
	_mhdebug("LANCE: reply ",ec->client);
...
message *reply_mess;
{
	_mhdebug("LANCE: mess_reply ",req->m_source);
