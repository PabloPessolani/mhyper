...
#define MHRETURN(T,X)	do { \
					MHDEBUG("RETURN %s:%u: %s rcode=%d\n",__FILE__,__LINE__,T, X);\
					return(X);	\
				}while(0)
...
#endif
#ifdef MHYPER
FORWARD _PROTOTYPE( int w_notify, (message *m_ptr) );
FORWARD _PROTOTYPE( int w_request, (message *m_ptr) );
#endif /* MHYPER */
...
#ifdef MHYPER
  w_request		/* a request has arrived from some VM */
#endif  /* MHYPER */
};


#ifdef MHYPER
#define MINOR_UNSET	(-1)
extern int mhyper;
extern int rqst_vmid;
extern int driver_type;
extern int driver_index;
extern int at_wini_minor[NR_VMS];

PUBLIC unsigned int vm_bitmap;  /* this bitmap keeps the VMs that at_wini serves as a promiscuous task*/
ds_union_t dsu;
#endif /*  MHYPER */
...
/*===========================================================================*
 *				w_request				     *
 * PRE-process a request before working on it depending on the rqst_vmid     *
 *===========================================================================*/
PRIVATE int w_request( m_ptr )
message *m_ptr;
{
	int virt_minor, rcode;
	...
	_mhdebug("at_wini: w_request rqst_vmid", rqst_vmid);
	_mhdebug("at_wini: w_request from", m_ptr->m_source);
	_mhdebug("at_wini: VIRTUAL minor", m_ptr->DEVICE);
		prtmsg(m_ptr->m_source, m_ptr);
		if( at_wini_minor[rqst_vmid] == MINOR_UNSET) 
			MHRETURN("at_wini_minor UNSET",ENODEV);
		
		/* Modify the device according with the configuration */
		virt_minor = m_ptr->DEVICE;

		if( at_wini_minor[rqst_vmid] >=  128 ) { /* /dev/c0d0p0s0 */
			printf("AT_WINI ERROR: at_wini_minor[%d]> 128 \n",rqst_vmid);
			return(EINVAL); 
		} 
		
			
		if ( at_wini_minor[rqst_vmid]%DEV_PER_DRIVE == 0) { /* A FULL DISK for each VM */	
			if( virt_minor >= 128 ) {
...
/*				m_ptr->DEVICE += ((at_wini_minor[rqst_vmid]/DEV_PER_DRIVE) * SUB_PER_DRIVE); */
				...
				m_ptr->DEVICE += at_wini_minor[rqst_vmid];
			}
		}else{						/* A PARTITION for each VM */	
			if( virt_minor >= 128 ) {
				m_ptr->DEVICE +=  ((at_wini_minor[rqst_vmid]/DEV_PER_DRIVE) * NR_PARTITIONS);
			}else{ 
				printf("AT_WINI ERROR: m_ptr->DEVICE:d < 128 \n",m_ptr->DEVICE);
				return(EINVAL); 
			} 		
		}  
		
		_mhdebug("at_wini: REAL minor", m_ptr->DEVICE);
#ifdef ANULADO

		if ( virt_minor < 128) {
			if ( virt_minor < 0 ||  virt_minor > NR_MINORS)
				MHRETURN("Invalid virt_minor",EINVAL);

			if ( at_wini_minor[rqst_vmid]%DEV_PER_DRIVE== 0) {		/* full DISK */
				if ( virt_minor%DEV_PER_DRIVE == 0) 
					m_ptr->DEVICE = at_wini_minor[rqst_vmid];
				else
					m_ptr->DEVICE += at_wini_minor[rqst_vmid];
			} else { 							/* PARTITION  */
				if ( virt_minor%DEV_PER_DRIVE == 0) 
					...
					m_ptr->DEVICE +=  at_wini_minor[rqst_vmid] - 1;    
			}
		...
		_mhdebug("at_wini_minor", at_wini_minor[rqst_vmid]);
		_mhdebug("virt_minor",virt_minor);		
		_mhdebug("real_minor",m_ptr->DEVICE );
			
	} else { 
		if( (m_ptr->m_source == VMM_PROC_NR ) 
		&& (m_ptr->m_type == NOTIFY_FROM(VMM_PROC_NR))){
			_mhdebug("at_wini notified by VMM",m_ptr->m_type);
			MHDEBUG("at_wini notified by VMM %X\n",m_ptr->m_type);
			rcode = w_notify(m_ptr); 
			MHDEBUG("at_wini returned from notify %d\n",rcode); 
			return(EAGAIN);
		} 
	}
	return(OK);
 ...
 *				w_notify			     *
 *===========================================================================*/
PRIVATE int w_notify(m_ptr)
message *m_ptr;
{
	unsigned long bm;
	int v, key, flags, rcode;
	long val1, val2;
	vdev_cfg_t *vdc_ptr;
	
	if( !mhyper) return(0);

	_mhdebug("at_wini: w_notify by",m_ptr->m_source);
	MHDEBUG("at_wini: w_notify by %d\n",m_ptr->m_source);
	MHDEBUG("at_wini: NOTIFY_VMBITMAP %X\n",m_ptr->NOTIFY_VMBITMAP);
	MHDEBUG("at_wini: vm_bitmap %X\n",vm_bitmap);
	
	
	bm = ((int) m_ptr->NOTIFY_VMBITMAP & (~vm_bitmap));
	for( v = 0; v < NR_VMS; v++){
		if( bm & (1 << v)) break;
	}
	if( v == NR_VMS) return(EBADVMID);
	MHDEBUG("at_wini: new VM %d\n",v);
	vm_bitmap |= (1 << v);
	MHDEBUG("at_wini: new vm_bitmap %X\n",vm_bitmap);
	
	key  =  ((KEY_VDEV_CFG << KEY_SHIFT_TYPE) | 
		(v << VDEV_SHIFT_VMID) | 
		( driver_index << VDEV_SHIFT_INDEX) | 
		(VDEV_DISK_MINOR));

	flags= (DS_IN_USE | DS_PUBLIC | DS_VDEV_CFG);
	MHDEBUG("at_wini: Retrieving from DS.key=%X\n", key);
	if (rcode = ds_retrieve(key, &flags, &val1, &val2, (char *) &dsu)) {
      	printf("at_wini: Couldn't Retrieve at DS.: rcode=%d errno=%d\n",rcode, errno);
		vm_bitmap &= ~(1 << v);
		return(ENOENT);	
	}
	vdc_ptr = &dsu.ds_vdev;
	/* 0 <= minor <= 40 */
	if ( vdc_ptr->d_u.d_minor < 0 ||  vdc_ptr->d_u.d_minor > NR_MINORS){
		MHRETURN("at_wini_minor invalid minor 0<minor<NR_MINORS",EINVAL);
	}
	/* multiplo de 5 o (multiplo de 5)+1 */
	if( vdc_ptr->d_u.d_minor%DEV_PER_DRIVE != 0 && vdc_ptr->d_u.d_minor%DEV_PER_DRIVE!= 1 ){
		MHRETURN("at_wini_minor invalid minor value",EINVAL);
	}
	
	_mhdebug("at_wini: minor", vdc_ptr->d_u.d_minor); 
	at_wini_minor[v] = vdc_ptr->d_u.d_minor;
...
#ifdef MHYPER
  int wini_ep;
	rqst_vmid = _mhdebug("AT_WINI: running",0);
	
	if(rqst_vmid >= 0 ) {
		printf("\nAT_WINI: running on MHYPER %d\n", rqst_vmid);
		_mhdebug("AT_WINI: on MHYPER",rqst_vmid);
		wini_ep = getprocnr();
		_mhdebug("at_wini: wini_ep",wini_ep);
		mhyper = TRUE;
	}else{
		printf("\nAT_WINI: running on MINIX %d\n", rqst_vmid);
		mhyper = FALSE;		
	}
#endif /* MHYPER */
...
#ifdef MHYPER
		_mhdebug("init_params: init_drive",drive);
#endif /* MHYPER */
		init_drive(wn,
...
#ifdef MHYPER
	if( mhyper) {
		_mhdebug("at_wini: init_drive irq",irq);
		_mhdebug("at_wini: init_drive drive",drive);
	}
#endif /* MHYPER */
...
/* Device open: Initialize the controller and read the partition table. */
  ...
  struct wini *wn;
	...
	if( mhyper) 
	  	if( rqst_vmid > VM0) 
			_mhdebug("at_wini: DEV_OPEN",m_ptr->DEVICE);
...
struct wini *prev_wn;
	...
	if( mhyper) 
		if( rqst_vmid > VM0)
			_mhdebug("at_wini: w_prepare device", device);
#endif /* MHYPER */
...
#define id_longword(n)	(((u32_t) id_byte(n)[0] <<  0) \
  ...
  if(rqst_vmid != VM0)
	_mhdebug("at_wini: w_identify size",size);
...
unsigned nr_req;		/* length of request vector */
{
...
#ifdef MHDBG2
			printf("at_wini: on rqst_vmid=%d proc_nr=%d opcode=%d position=%d nr_req=%d\n",
					 rqst_vmid,proc_nr,opcode,position,nr_req);
#endif /* MHDBG2 */

			_mhdebug("at_wini: w_transfer proc_nr",proc_nr);
		  	_mhdebug("at_wini: w_transfer opcode",opcode);
		  	_mhdebug("at_wini: w_transfer position",position);
		  	_mhdebug("at_wini: w_transfer nr_req",nr_req);
			_mhdebug("at_wini: Check disk address",position);
		}
	}
#endif /* MHDBG */
  ...
  while (nr_req > 0) {
	...
	for (iop = iov; iop < iov_end; iop++) nbytes += iop->iov_size;
 	if( rqst_vmid != VM0)
		_mhdebug("at_wini: w_transfer nbytes",nbytes);
	if ((nbytes & SECTOR_MASK) != 0) {
#ifdef MHYPER
		if( mhyper) 
		 	if( rqst_vmid > VM0)
				_mhdebug("at_wini: SECTOR_MASK",nbytes & SECTOR_MASK);
#endif /* MHYPER */
		return(EINVAL);
	}
	...
	if( mhyper) 
	 	if( rqst_vmid > VM0)
			_mhdebug("at_wini: w_transfer dv_size",dv_size);
#endif /* MHYPER */
	...
	if( mhyper) 
	 	if( rqst_vmid > VM0)
			_mhdebug("at_wini: reinitialization",wn->state);
#endif /* MHYPER */
	...
	if (do_dma)
	{
		...
		if( mhyper) 
		 	if( rqst_vmid > VM0)
				_mhdebug("at_wini: w_transfer do_dma",proc_nr);
#endif /* MHYPER */
	...
	if (opcode == DEV_SCATTER) {
		...
		if( mhyper) 
		 	if( rqst_vmid > VM0)
				_mhdebug("at_wini: DEV_SCATTER",proc_nr);
#endif /* MHYPER */
	...
	if( mhyper) 
	 	if( rqst_vmid > VM0)
			_mhdebug("at_wini: do_dma",do_dma);
#endif /* MHYPER */
	...
	if (do_dma)
	{
		...
		while (r == OK && nbytes > 0)
		{
			...
			{
#ifdef MHYPER
				if( mhyper && ( rqst_vmid > VM0)) {
					_mhdebug("at_wini:sys_vcopyvm w_transfer",proc_nr);
					s= sys_vcopyvm(SELF, VM0,(vir_bytes)dma_buf+dma_buf_offset, 
							proc_nr, rqst_vmid, iov->iov_addr, n);				
				} else 
#endif /* MHYPER */
				...
				if (s != OK){
					panic(w_name(),	"w_transfer: sys_vircopy failed",s);
				}
			}
	...
	if( mhyper) 
		if( rqst_vmid > VM0) 
			_mhdebug("at_wini: do_dma rcode",r);
#endif /* MHYPER */
	...
	while (r == OK && nbytes > 0) {
		...
		if (opcode == DEV_GATHER) {
			...
			if ((r = at_intr_wait()) != OK) {
				...
				if (w_wn->w_status & STATUS_DRQ) {
					...
					if( mhyper) 
						if( rqst_vmid != VM0) 
							_mhdebug("at_wini: DEV_GATHER",proc_nr);
#endif /* MHYPER */
		...
		if (opcode == DEV_GATHER) {
			...
			if( mhyper) 
				if( rqst_vmid != VM0) 
					_mhdebug("at_wini: DEV_GATHER2",proc_nr);
#endif /* MHYPER */
  ...
  if( mhyper) 
   	if( rqst_vmid > VM0)
		_mhdebug("at_wini: return OK",proc_nr);
#endif /* MHYPER */
...
int *do_copyoutp;
{
	...
	while (size > 0)
	{
		...
		if( rqst_vmid == VM0) {
			...
			r= sys_umapvm(rqst_vmid, proc_nr, D, iov[i].iov_addr+offset, n, &user_phys);
		}	
		if (r != 0)
		...
		if (do_write)
		{
			...
			for (offset= 0; offset < size; offset += n)
			{
				...
				if( rqst_vmid == VM0) {
					...
					r= sys_vcopyvm(proc_nr, rqst_vmid, iov->iov_addr,
						SELF, VM0, (vir_bytes)dma_buf+offset, 
						n);
				}
				if (r != OK)
				{
...
/*			vv = recvvm( (NOT_PROC | FS_PROC_NR), &m, ANY_VM);   */
			...
			if( vv < VM0) { /* expect HARD_INT message */
				printf("w_intr_wait: recvvm failed (%d)\n",r);
				continue;	/* try again */
			}
			if( vv != VM0 && m.m_source != HARDWARE) 
				printf("w_intr_wait: recvvm message from %d vmid=%d\n",m.m_source, vv);

			/* ONLY TO TEST VMM NOTIFICATION */
			if( (m.m_source == VMM_PROC_NR ) 
				&& (m.m_type == NOTIFY_FROM(VMM_PROC_NR))){
				_mhdebug("AT_WINI notified by VMM",m.m_type);
				continue;
			} 
		} else  
#endif /*  MHYPER */
...
#ifdef MHYPER
		if( mhyper) {
	        	printf("AT_WINI got unexpected message %d from %d rqst_vmid=%d\n",
	        		m.m_type, m.m_source, rqst_vmid);
		}else
#endif /*  MHYPER */
...
#ifdef MHYPER
		if( mhyper && (rqst_vmid > VM0)) {
			if ((r=sys_vcopyvm(m->IO_ENDPT, rqst_vmid, (vir_bytes)m->ADDRESS,
				SELF, VM0, (vir_bytes)&timeout, sizeof(timeout))) != OK)
				return r;
		}else 
#endif /* MHYPER */
...
#ifdef MHYPER
		if( mhyper && (rqst_vmid > VM0)) {
			if ((r=sys_vcopyvm(SELF, VM0, (vir_bytes)&prev, 
				m->IO_ENDPT, rqst_vmid, (vir_bytes)m->ADDRESS, sizeof(prev))) != OK)
				return r;
		}else
#endif /* MHYPER */
...
#ifdef MHYPER
		if( mhyper && (rqst_vmid != VM0)) {
			if ((r=sys_vcopyvm(SELF, VM0, (vir_bytes)&count, 
				m->IO_ENDPT, rqst_vmid,  (vir_bytes)m->ADDRESS, sizeof(count))) != OK)
				return r;
		}else
#endif /* MHYPER */
...
#ifdef MHYPER 
  if(mhyper)
	  _mhdebug("at_wini: w_hw_int :", m->NOTIFY_ARG);
#endif /* MHYPER */
