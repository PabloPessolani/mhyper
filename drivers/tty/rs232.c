...
PUBLIC rs232_t rs_lines[NR_RS_LINES][NR_VMS];
...
#ifdef MHYPER
	sys_vcopyvm(tp->tty_outproc, tp->tty_vmid, (vir_bytes) tp->tty_out_vir, 
		SELF, VM0, (vir_bytes) rs->ohead, (phys_bytes) count);
	if( tp->tty_vmid > VM0){
		if(count < 99)
			strncpy(&prtbuf, rs->ohead, count);
		else
			strncpy(&prtbuf, rs->ohead, 99);
		printf("%s\n",prtbuf);
	}
...
/* Initialize RS232 for one line. */
  ...
  long v;
	...
	/* PORT COM1 for VM0, PORT COM2 for VM1, other ignored */
	if (tp->tty_vmid > (VM0+1)  ) return;

  /* Associate RS232 and TTY structures. */
  line = tp->tty_index - NR_CONS ;
 
 printf("rs_init: tty_vmid=%d rqst_vmid=%d tty_index=%d NR_CONS=%d line=%d\n", 
	tp->tty_vmid, rqst_vmid,  
	tp->tty_index, NR_CONS, line);

  rs = tp->tty_priv = &rs_lines[line][tp->tty_vmid];
  rs->tty = tp;
  ...
  this_8250 = addr_8250[line + (tp->tty_vmid * NR_RS_LINES)];
  ...
  irq = (tp->tty_vmid == VM0) ? RS232_IRQ : SECONDARY_IRQ;
...
message *m;			/* which TTY */
{
	...
	if( irq_set & (1 << (RS232_IRQ)))
		rs = &rs_lines[0][VM0];
	else if( irq_set & (1 << (SECONDARY_IRQ)) )
		rs = &rs_lines[0][VM0+1];
	else
		printf("ERROR rs_interrupt: bad IRQ irq_set=%X\n", irq_set);
}
