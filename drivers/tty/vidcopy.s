...
.define	_vid_mem_copy	! copy video ram to data ram
...
!*				vid_mem_copy				     *
!*===========================================================================*
! PUBLIC void vid_mem_copy(unsigned src_video, u16 *dst_ram,  unsigned count);
!
_vid_mem_copy:
	push	ebp
	mov	ebp, esp
	...
	push	es
	push	fs
	mov	esi, 8(ebp)			! src VIDEO
	mov	edi, 12(ebp)		! destination RAM
	mov	edx, 16(ebp)		! count
	mov	fs, (_vid_seg)		! segment containing video memory
...
vmc_loop:
	...
	mov	ecx, eax		! ecx = min(ecx, vid_size - esi)
0:	sub	edx, ecx		! count -= ecx
	shl	esi, 1			! byte address
	add	esi, (_vid_off)	! in video memory
vmc_copy:
	push ds
	pop  es				! ds<->es (ram)
	rep					! copy words from video memory
	fseg o16 movs		! FS:ESI->ES:EDI
vmc_test:
	sub	esi, (_vid_off)
	shr	esi, 1			! back to a word address
	test	edx, edx
	jnz	vmc_loop
vmc_done:
	pop fs
