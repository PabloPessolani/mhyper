...
#define tty_addr(l, v)	&tty_table[v].line[l]

/* Macros for magic tty types. */
#define isconsole(tp)	((tp->tty_index) < (NR_CONS))
#define ispty(tp)		((tp->tty_index) >= (NR_CONS+NR_RS_LINES))

/* Macros for magic tty structure pointers. */
#define FIRST_TTY(vmid)		(&tty_table[vmid].line[0]) 
#define END_TTY(vmid)		(&tty_table[vmid].line[NR_CONS+NR_RS_LINES+NR_PTYS])
...
FORWARD _PROTOTYPE( void tty_init, (int vmid)				);
#if ENABLE_SRCCOMPAT || ENABLE_BINCOMPAT
...
PUBLIC tty_tab_t tty_table[NR_VMS];
...
PUBLIC unsigned int vm_bitmap;  /* this bitmap keeps the VMs that TTY serves as a promiscuous task*/
PUBLIC struct tty *tty_head; 		/* head of the linked list of ttys with pending events */
PUBLIC struct tty *tty_tail; 		/* tail of the linked list of ttys with pending events */
#endif /*  MHYPER */
...
/* Main routine of the terminal task. */
  ...
  message tty_mess, *m_ptr;		/* buffer for all incoming messages */
  unsigned line;
  int r, s, v;
  ...
  register tty_t *tp;
	...
	vm_bitmap = 0;
	tty_vmid = _mhdebug("TTY: vmid=", tty_vmid);
	tty_vmid = _mhdebug("TTY: vmid=", tty_vmid);
#endif /*  MHYPER */ 
  ...
  while (TRUE) {
	...
	while ( (tp = tty_dequeue()) != NULL) 
		if (tp->tty_events) 
			handle_events(tp);
	
	/* Get a request message. */
	rqst_vmid= recvvm(ANY, &tty_mess, ANY_VM);
	if (rqst_vmid < VM0) {
		_mhdebug("TTY receive failed rqst_vmid", rqst_vmid);
		panic("TTY", "receive failed with %d", rqst_vmid);
	}
#ifdef TTYDBG
	if (rqst_vmid > VM0) {
		m_ptr =  &tty_mess;
		_mhdebug("TTY rqst_vmid",rqst_vmid);
		_mhdebug("TTY tty_mess.m_type",tty_mess.m_type);
		prtmsg(m_ptr->m_source, m_ptr);		
	}
#endif
	/*Verify if the request came from a registered VM */
	if( !(vm_bitmap & (1 << rqst_vmid)) ){
		_mhdebug("TTY vm_bitmap",vm_bitmap);
		_mhdebug("TTY INVALID request from",rqst_vmid);
		continue;
	}
	if( (tty_mess.m_source == VMM_PROC_NR ) 
	 && (tty_mess.m_type == NOTIFY_FROM(VMM_PROC_NR))){
		m_ptr =  &tty_mess;
		_mhdebug("TTY tty_mess.m_type",tty_mess.m_type);
		_mhdebug("TTY tty_mess.NOTIFY_VMBITMAP", (unsigned int) tty_mess.NOTIFY_VMBITMAP);
		_mhdebug("TTY vm_bitmap",vm_bitmap);
		new_vm = ~vm_bitmap & (unsigned int) tty_mess.NOTIFY_VMBITMAP;
		if( new_vm) {
			for( v = 0; v < NR_VMS ; v++){
				if( new_vm == (1 << v) ) {
					new_vm = v;
					_mhdebug("TTY new_vm",new_vm);
					tty_init(new_vm);
					break;
				}
			}
	...
	case DEV_PING:
#ifdef MHYPER
		notifyvm(tty_mess.m_source, rqst_vmid);
#else /*  MHYPER */
	...
	if (line == KBD_MINOR) {
		...
		tp = tty_addr((line - CONS_MINOR), rqst_vmid);
	} else if (line == LOG_MINOR) {
		VMDEBUG("TTY line == LOG_MINOR",LOG_MINOR);
		tp = tty_addr(0, rqst_vmid);
	} else if ((line - RS232_MINOR) < NR_RS_LINES) {
		tp = tty_addr(line - RS232_MINOR + NR_CONS, rqst_vmid);
	} else if ((line - TTYPX_MINOR) < NR_PTYS) {
		tp = tty_addr(line - TTYPX_MINOR + NR_CONS + NR_RS_LINES, rqst_vmid);
	} else if ((line - PTYPX_MINOR) < NR_PTYS) {
		tp = tty_addr(line - PTYPX_MINOR + NR_CONS + NR_RS_LINES, rqst_vmid);
	...
	if (tp == NULL || ! tty_active(tp)) {
		...
		VMDEBUG("TTY got illegal request from",tty_mess.m_source);
	...
	if(rqst_vmid != VM0) {
		printf("TTY: m_type=%d tty_vmid=%d rqst_vmid=%d \n",
			tty_mess.m_type, tp->tty_vmid, rqst_vmid);
	}
#endif
...
message *m_ptr;
{
   ...
   */
  event_found = 0;
  for (tp = FIRST_TTY(rqst_vmid); tp < END_TTY(rqst_vmid); tp++) {
...
/* A process wants to read from a terminal. */
  ...
  int more_verbose= (tp == tty_addr(NR_CONS, tp->tty_vmid));
...
#ifdef MHYPER
	  if (sys_umapvm(tp->tty_vmid, m_ptr->IO_ENDPT, D, (vir_bytes) m_ptr->ADDRESS, m_ptr->COUNT,
		&phys_addr) != OK) {
#else
 ...
 */
  ...
  size_t size;
   ...
   _hdebug("do_ioctl: TTY_REQUEST", m_ptr->TTY_REQUEST);
...
#ifdef MHYPER
   _hdebug("do_ioctl: TCGETS vmid", tp->tty_vmid);
	r = sys_vcopyvm(SELF, VM0, (vir_bytes) &tp->tty_termios,
		m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS, 
		(vir_bytes) size);
#else /*  MHYPER */
...
#ifdef MHYPER
   _hdebug("do_ioctl: TCSETS vmid", tp->tty_vmid);
	r = sys_vcopyvm( m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS,
		SELF, VM0, (vir_bytes) &tp->tty_termios, (vir_bytes) size);
#else /*  MHYPER */
    ...
    case TCFLSH:
#ifdef MHYPER
   _hdebug("do_ioctl: TCFLSH vmid", tp->tty_vmid);
	r = sys_vcopyvm( m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS,
		SELF, VM0, (vir_bytes) &param.i, (vir_bytes) size);
#else /*  MHYPER */
	r = sys_vircopy( m_ptr->IO_ENDPT, D, (vir_bytes) m_ptr->ADDRESS,
    ...
    case TCFLOW:
#ifdef MHYPER
   _hdebug("do_ioctl: TCFLOW vmid", tp->tty_vmid);
	r = sys_vcopyvm( m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS,
		SELF, VM0, (vir_bytes) &param.i, (vir_bytes) size);
#else /*  MHYPER */
	r = sys_vircopy( m_ptr->IO_ENDPT, D, (vir_bytes) m_ptr->ADDRESS,
    ...
    case TIOCGWINSZ:
#ifdef MHYPER
   _hdebug("do_ioctl: TIOCGWINSZ vmid", tp->tty_vmid);
	r = sys_vcopyvm(SELF, VM0, (vir_bytes) &tp->tty_winsize,
		m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS, 
		(vir_bytes) size);
#else /*  MHYPER */
    ...
    case TIOCSWINSZ:
#ifdef MHYPER
   _hdebug("do_ioctl: TIOCSWINSZ vmid", tp->tty_vmid);
	r = sys_vcopyvm( m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS,
		SELF, VM0, (vir_bytes) &tp->tty_winsize, (vir_bytes) size);
#else /*  MHYPER */
	r = sys_vircopy( m_ptr->IO_ENDPT, D, (vir_bytes) m_ptr->ADDRESS,
...
#ifdef MHYPER
   _hdebug("do_ioctl: TIOCGETP vmid", tp->tty_vmid);
	r = sys_vcopyvm(SELF, VM0, (vir_bytes) &param.sg,
		m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS,
		(vir_bytes) size);
#else /*  MHYPER */
    ...
    case TIOCSETP:
#ifdef MHYPER
   _hdebug("do_ioctl: TIOCSETP vmid", tp->tty_vmid);
	r = sys_vcopyvm( m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS,
		SELF, VM0, (vir_bytes) &param.sg, (vir_bytes) size);
#else /*  MHYPER */
	r = sys_vircopy( m_ptr->IO_ENDPT, D, (vir_bytes) m_ptr->ADDRESS,
...
#ifdef MHYPER
   _hdebug("do_ioctl: TIOCGETC vmid", tp->tty_vmid);
	r = sys_vcopyvm(SELF, VM0, (vir_bytes) &param.tc,
		m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS, 
		(vir_bytes) size);
#else /*  MHYPER */
    ...
    case TIOCSETC:
#ifdef MHYPER
   _hdebug("do_ioctl: TIOCSETC vmid", tp->tty_vmid);
	r = sys_vcopyvm( m_ptr->IO_ENDPT, tp->tty_vmid, (vir_bytes) m_ptr->ADDRESS,
		SELF, VM0, (vir_bytes) &param.tc, (vir_bytes) size);
#else /*  MHYPER */
	r = sys_vircopy( m_ptr->IO_ENDPT, D, (vir_bytes) m_ptr->ADDRESS,
 ...
 */
  ...
  if (m_ptr->TTY_LINE == LOG_MINOR) {
	...
	VMDEBUG("do_open LOG_MINOR",LOG_MINOR);
...
#ifdef MHYPER
		notifyvm(tp->tty_select_proc, tp->tty_vmid);
#else /*  MHYPER */
 ...
 */
  char *buf;
  unsigned count;
  int status, saved_vmid ;


  saved_vmid = rqst_vmid;
  rqst_vmid = tp->tty_vmid;
...
#ifdef MHYPER
		notifyvm(tp->tty_incaller, tp->tty_vmid);
#else /*  MHYPER */
...
#ifdef MHYPER
			sys_vcopyvm(SELF, VM0, (vir_bytes) buf, 
				tp->tty_inproc, tp->tty_vmid, tp->tty_in_vir,
...
#ifdef MHYPER
	sys_vcopyvm(SELF, VM0, (vir_bytes) buf, 
		tp->tty_inproc, tp->tty_vmid, tp->tty_in_vir, (vir_bytes) count);
#else /*  MHYPER */
	sys_vircopy(SELF, D, (vir_bytes) buf, 
...
#ifdef MHYPER
	result = sys_vcopyvm(tp->tty_ioproc, tp->tty_vmid, tp->tty_iovir,
			SELF, VM0, (vir_bytes) &tp->tty_termios,
...
/* Send a reply to a process that wanted to read or write data. */
  ...
  tty_mess.REP_STATUS = status;
	...
	if (rqst_vmid > VM0) {
		m_ptr =  &tty_mess;
		_mhdebug("tty_reply to ", replyee);
		prtmsg(TTY_PROC_NR, m_ptr);
	}
	
  if ((status = sendvm(replyee, &tty_mess, rqst_vmid)) != OK) {
 ...
 *				tty_dequeue					     * 
 *===========================================================================*/
PUBLIC tty_t *tty_dequeue(void)
{
	tty_t *tp;

	if( tty_head == NULL) return(NULL);

	tp = tty_head;

	tty_head = tp->tty_next;
	if( tty_head == NULL)
		tty_tail = NULL;
	return(tp);
}
...
#ifdef MHYPER  
	if (tp->tty_pgrp != 0) {
		_mhdebug("tty sigchar sys_killvm", tp->tty_pgrp);	
		if (OK != (status = sys_killvm(tp->tty_pgrp, sig,tp->tty_vmid)))
			panic("TTY","Error, call to sys_killvm failed", status);
	}
#else /* MHYPER */
	if (tp->tty_pgrp != 0) {
		_mhdebug("tty sigchar sys_kill", tp->tty_pgrp);	
 ...
 *				tty_init				     *
 *===========================================================================*/
PRIVATE void tty_init(int vmid)
{
...
#ifdef MHYPER
	vm_bitmap |= (1 << vmid);
	_mhdebug("TTY tty_init vm_bitmap",vm_bitmap);
#endif /*  MHYPER */

  /* Initialize the terminal lines. */
  for (tp = FIRST_TTY(vmid),s=0; tp < END_TTY(vmid); tp++,s++) {
	...
	tp->tty_next = NULL;	
  	...
  	if (tp->tty_index < NR_CONS) {						/* CONSOLE */
		scr_init(tp);
		kb_init(tp);
  		tp->tty_minor = CONS_MINOR + s;
  	} else 	if (tp->tty_index < (NR_CONS+NR_RS_LINES)) {	/* RS232  */
		rs_init(tp);
...
/* This timer has expired. Set the events flag, to force processing. */
  tty_t *tty_ptr;
  tty_ptr = &tty_table[tmr_arg(tp)->ta_int].line[rqst_vmid];
 ...
 */
  ...
  sendvm(m_ptr->m_source, &reply_mess, tp->tty_vmid);
}
