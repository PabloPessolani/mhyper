...
typedef struct console {
  ...
  char c_screen[MAX_COLS * MAX_ROWS * 2];
} console_t;

PUBLIC int nr_cons= 1;		/* actual number of consoles */
...
PRIVATE	phys_bytes phys_video;	/* physical address of VIDEO memory */
PRIVATE phys_bytes vid_base;
PRIVATE unsigned page_size;

PUBLIC 	int ccurrent;				/* # currently visible console */
PUBLIC 	int vmcurrent;				/* # currently VM screens */
...
/* FORWARD _PROTOTYPE( void mem_vid_copy2, (vir_bytes src, int dst_index, int count)); */
...
#ifdef MHYPER
	if ((result = sys_vcopyvm(tp->tty_outproc, tp->tty_vmid, tp->tty_out_vir, 
				SELF, VM0, (vir_bytes) buf, (vir_bytes) count)) != OK)
#else /*  MHYPER */
...
int dir;			/* SCROLL_UP or SCROLL_DOWN */
{
   ...
   */
if( cons->c_vmid == vmcurrent) {    /* Scroll the console of CURRENT VMID */
  if (dir == SCROLL_UP) {
  ...
  flush(cons);
} else {				/* Scroll a BACKGROUND console */
  if (dir == SCROLL_UP) {
	/* Scroll one line up in 3 ways: soft, avoid wrap, use origin. */
	if (softscroll) {
		memcpy( &cons_table[cons->c_vmid].vscreen[cons->c_start*2], 
				&cons_table[cons->c_vmid].vscreen[(cons->c_start + scr_width)*2],  
				chars*2);
	} else
	if (!wrap && cons->c_org + scr_size + scr_width >= cons->c_limit) {
		memcpy( &cons_table[cons->c_vmid].vscreen[cons->c_start*2], 
				&cons_table[cons->c_vmid].vscreen[ (cons->c_org + scr_width) *2],  
				chars*2);
		cons->c_org = cons->c_start;
	} else {
		cons->c_org = (cons->c_org + scr_width) & vid_mask;
	...
	/* Scroll one line down in 3 ways: soft, avoid wrap, use origin. */
	if (softscroll) {
		memcpy( &cons_table[cons->c_vmid].vscreen[(cons->c_start + scr_width)*2], 
				&cons_table[cons->c_vmid].vscreen[cons->c_start*2],  
				chars*2);
	} else
	if (!wrap && cons->c_org < cons->c_start + scr_width) {
		new_org = cons->c_limit - scr_size;
		memcpy( &cons_table[cons->c_vmid].vscreen[(new_org + scr_width)*2], 
				&cons_table[cons->c_vmid].vscreen[cons->c_org *2],  
		 		chars*2);
		cons->c_org = new_org;
	} else {
		cons->c_org = (cons->c_org - scr_width) & vid_mask;
  ...
  /* Blank the new line at top or bottom. */
  blank_color = cons->c_blank;
  memset(&cons_table[cons->c_vmid].vscreen[new_line*2], 0, scr_width*2);
 ...
 */
  ...
  if (cons->c_rwords > 0) {
    if( cons->c_vmid == vmcurrent)  { /*   Scroll the console of CURRENT VMID */
		...
		memcpy( &cons_table[cons->c_vmid].vscreen[cons->c_cur*2],
				cons->c_ramqueue, 
				cons->c_rwords * 2 );
	}
	cons->c_rwords = 0;
  ...
  if (cons->c_esc_intro == '[') {
	...
	switch (c) {
		...
		if( cons->c_vmid == vmcurrent) 
			...
			memset(&cons_table[cons->c_vmid].vscreen[dst*2], 0, count*2);
		break;
		...
		if( cons->c_vmid == vmcurrent) { 
			...
			memcpy( &cons_table[cons->c_vmid].vscreen[dst*2],
					&cons_table[cons->c_vmid].vscreen[src*2], 
					count*2);
			blank_color = cons->c_blank;
			memset(&cons_table[cons->c_vmid].vscreen[src*2], 0, n * scr_width *2);
		}
		break;
		...
		if( cons->c_vmid == vmcurrent) { 	
			...
			memcpy( &cons_table[cons->c_vmid].vscreen[dst*2], 
					...
					count*2);
			blank_color = cons->c_blank;
			memset(&cons_table[cons->c_vmid].vscreen[src*2], 0, n*2);
		}
		break;
		...
		if(1) { /* cons->c_vmid == vmcurrent) {*/
			...
			memcpy( &cons_table[cons->c_vmid].vscreen[dst*2], 
					...
					count*2);
			blank_color = cons->c_blank;
			memset(&cons_table[cons->c_vmid].vscreen[(dst+count)*2], 0, n*2);
		}
		break;
...
PUBLIC void do_video(message *m)
{
	...
	unsigned char c;
  	...
  	_mhdebug("do_video rqst_vmid", rqst_vmid);
...
#ifdef MHYPER
			r= sys_vcopyvm(m->IO_ENDPT, rqst_vmid,
				(vir_bytes)m->ADDRESS,
				SELF, VM0, (vir_bytes)&mapreq, sizeof(mapreq));
#else /*  MHYPER */
...
/* Initialize the screen driver. */
  console_t *cons;
  u16_t bios_columns, bios_crtbase, bios_fontlines, bios_mode;
  u8_t bios_rows;
  int line, vmid;
  int s;
...
#ifdef MHYPERXXX
 	_mhdebug("scr_init: line",tp->tty_index);
  	_mhdebug("scr_init: vmid",tp->tty_vmid);
#endif /*  MHYPERXXX */
  ...
  vmid = tp->tty_vmid;
  if (vmid >= NR_VMS) return;
  
  cons = &cons_table[vmid].ct[line];
  ...
  if (! vdu_initialized++) {
	...
	/* How about error checking? What to do on failure??? */
  	...
  	s=sys_vircopy(SELF, BIOS_SEG, (vir_bytes) VDU_VIDEO_MODE_ADDR, 
  		SELF, D, (vir_bytes) &bios_mode, VDU_VIDEO_MODE_SIZE);
	...
	if( s < 0)
    	_mhdebug("scr_init: sys_umap phys_video", phys_video);
  	...
  	/* There can be as many consoles (by VM) as video memory allows. */
...
#ifdef MHYPER
  	_mhdebug("scr_init: vid_size",vid_size);
	_mhdebug("scr_init: scr_size",scr_size);
	_mhdebug("scr_init: nr_cons",nr_cons);
	_mhdebug("scr_init: page_size",page_size);
	_mhdebug("scr_init: cons->c_start",cons->c_start);
	_mhdebug("scr_init: cons->c_limit",cons->c_limit);
	
/*
  	_mhdebug("TTY scr_init vid_index",vid_index);
	_mhdebug("TTY scr_init vid_seg",vid_seg);
	_mhdebug("TTY scr_init vid_off",vid_off);
	_mhdebug("TTY scr_init bios_mode",bios_mode);
*/
  ...
  if (line != 0) {
	...
	if( 1) /*vmid == vmcurrent) */
		...
		memcpy(&cons_table[cons->c_vmid].vscreen[cons->c_start*2], 0, scr_size*2);
  } else {
	int i, n;
...
#ifdef MHYPER
      notifyvm(LOG_PROC_NR, vmcurrent);
#else /*  MHYPER */
      notify(LOG_PROC_NR);
#endif /*  MHYPER */
...
static char diagbuf[100]; 
...
#ifdef MHYPER
		if (sys_vcopyvm(proc_nr, rqst_vmid, src++, SELF, VM0, (vir_bytes) &c, 1) != OK) {
			result = EFAULT;
			break;
		}
		diagbuf[m_ptr->DIAG_BUF_COUNT-count] = c;
#else /*  MHYPER */
  ...
  diagbuf[m_ptr->DIAG_BUF_COUNT] = 0;
  _mhdebug(diagbuf, m_ptr->DIAG_BUF_COUNT);
  
  m_ptr->m_type = result;
  sendvm(m_ptr->m_source, m_ptr, rqst_vmid);
}
...
/* Provide the log device with debug output */
  ...
  int r;
  	...
  	_mhdebug("do_get_kmess rqst_vmid", rqst_vmid);

  dst = (vir_bytes) m_ptr->GETKM_PTR;
  r= OK;
#ifdef MHYPER
  if (sys_vcopyvm(SELF, VM0, (vir_bytes)&kmess, m_ptr->m_source, rqst_vmid,
	dst, sizeof(kmess)) != OK) {
	r = EFAULT;
  }
#else /*  MHYPER */
 ...
 */
  if (c != 0) {
	if (c == '\n') cons_putk('\r');
	out_char(&cons_table[VM0].ct[0], (int) c);
  } else {
	flush(&cons_table[VM0].ct[0]);
  }
}
...
/* Prepare for halt or reboot. */
  cons_org0();
  softscroll = 1; 
  select_console(0, VM0);
  cons_table[VM0].ct[0].c_attr = cons_table[VM0].ct[0].c_blank = BLANK_COLOR;
}
...
/* Scroll video memory back to put the origin at 0. */
  ...
  for (cons_line = 0; cons_line < nr_cons; cons_line++) {
    cons = &cons_table[rqst_vmid].ct[cons_line];
	while (cons->c_org > cons->c_start) {
		...
		if( rqst_vmid == vmcurrent) 
			vid_vid_copy(cons->c_org, cons->c_org - n, scr_size);
		else
			memcpy( &cons_table[cons->c_vmid].vscreen[(cons->c_org - n)*2], 
					&cons_table[cons->c_vmid].vscreen[cons->c_org *2], 
					scr_size*2);
		cons->c_org -= n;
	}
  ...
  select_console(ccurrent, vmcurrent);
}

/*===========================================================================*
 *				select_console				     *
 *  VER https://github.com/minix3/minix/blob/master/drivers/tty/arch/i386/console.c *
 *===========================================================================*/
PUBLIC void select_console(int cons_line, int vmid)
{
/* Set the current console to console number 'cons_line'. */
	int s;

	_mhdebug("select_console: cons_line", cons_line);
	_mhdebug("select_console:  vmid",  vmid);
	
	if ( cons_line < 0 || cons_line >= nr_cons) return;
	if ( vmid    < VM0 || vmid      >= NR_VMS)  return;
  
	if ( vmid != vmcurrent) {
		_mhdebug("select_console: NEW vmid", vmid);

		/* Save video memory on VM's virtual screen  +scr_width */
		vid_mem_copy(0, (u16_t *) cons_table[vmcurrent].vscreen, vid_size); 

		/* save the VM's current console  */
		cons_table[vmcurrent].cline = ccurrent;
				
		/* New current VM */
		vmcurrent = vmid;
		
		/* Restore new VM's screen  */
		mem_vid_copy((u16_t *) cons_table[vmcurrent].vscreen, 0,  vid_size); 
		
		/* Restore new VM's current console */
		ccurrent   = cons_table[vmcurrent].cline;
	} else if (ccurrent != cons_line) {
		_mhdebug("select_console: NEW cons_line", cons_line);
		ccurrent = cons_line;
	}

	curcons = &cons_table[vmcurrent].ct[ccurrent];
	_mhdebug("select_console: ccurrent", ccurrent);
 ...
 *				print_console				     *
 *===========================================================================*/
void print_console(cons, vmid)
int cons;
int vmid;	
{
   	_mhdebug("print_console: vmid", vmid);

	mem_vid_copy((u16_t *) cons_table[vmid].vscreen, 0,  scr_size); 
