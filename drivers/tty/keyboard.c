...
message *m;
{
	...
	switch (m->m_type) {
...
#ifdef MHYPER
		r= sys_vcopyvm(SELF, VM0, (vir_bytes)&kbdp->buf[kbdp->offset], 
			m->IO_ENDPT, rqst_vmid, (vir_bytes) m->ADDRESS, n);
#else /*  MHYPER */
		...
		{
#ifdef MHYPER
			r= sys_vcopyvm(m->IO_ENDPT, rqst_vmid, (vir_bytes) m->ADDRESS+i,
				SELF, VM0, (vir_bytes)&c, 1);
#else /*  MHYPER */
		...
		if (kbdp == &kbd && m->TTY_REQUEST == KIOCSLEDS)
		{
			...
			unsigned char b;
#ifdef MHYPER
			r= sys_vcopyvm(m->IO_ENDPT, rqst_vmid, (vir_bytes) m->ADDRESS,
				SELF, VM0, (vir_bytes)&leds, sizeof(leds));
#else /*  MHYPER */
...
#ifdef MHYPER
			r= sys_vcopyvm(m->IO_ENDPT, rqst_vmid, (vir_bytes) m->ADDRESS,
				SELF, VM0, (vir_bytes)&bell, sizeof(bell));
#else /*  MHYPER */
			r= sys_vircopy(m->IO_ENDPT, D, (vir_bytes) m->ADDRESS,
...
#ifdef MHYPER
		r= sys_vcopyvm(SELF, VM0, (vir_bytes)&kbdp->buf[kbdp->offset], 
			kbdp->req_proc, rqst_vmid, kbdp->req_addr, n);
#else /*  MHYPER */
		r= sys_vircopy(SELF, D, (vir_bytes)&kbdp->buf[kbdp->offset], 
...
/* A keyboard interrupt has occurred.  Process it. */
  ...
  if (kbdp)
  {
	...
	if (kbdp->avail >= KBD_BUFSZ)
	{
	 ...
	 if (kbdp->req_size)
#ifdef MHYPER
		notifyvm(kbdp->incaller, kbdp->invmid);
#else /*  MHYPER */
		notify(kbdp->incaller);
#endif /*  MHYPER */	 
	 if (kbdp->select_ops & SEL_RD)
#ifdef MHYPER
		notifyvm(kbdp->select_proc, kbdp->select_vmid);
#else /*  MHYPER */
  ...
  if (icount < KB_IN_BYTES) {
	*ihead++ = scode;
	...
	icount++;
	tty_table[vmcurrent].line[ccurrent].tty_events = 1;
	tty_enqueue(&tty_table[vmcurrent].line[ccurrent]);
	if (tty_table[vmcurrent].line[ccurrent].tty_select_ops & SEL_RD) {
		select_retry(&tty_table[vmcurrent].line[ccurrent]);
	}
  }
}
...
/* Process characters from the circular keyboard buffer. */
  ...
  tp = &tty_table[vmcurrent].line[ccurrent];		/* always use the current console */
  ...
  while (icount > 0) {
	...
	if (ch == ALEFT) {
		...
		if(ccurrent > 0)
			select_console(ccurrent - 1, vmcurrent);
		else 
			select_console(nr_cons - 1, vmcurrent);
		set_leds();
	} else
	if (ch == ARIGHT) {
		...
		if(ccurrent < (nr_cons - 1))
			...
			select_console( 0, vmcurrent);		
		set_leds();
	} else
	if (ch == AUP) {
		/* Choose higher numbered VM  */
		_mhdebug("KEYBOARD AUP",vm_bitmap);
		bm = vm_bitmap;
		bm &= ~(1 << vmcurrent);
		if( bm > (1 << vmcurrent) ) { 
			new_vm = (-1);
			for( v = vmcurrent+1; v < NR_VMS; v++){
				if( vm_bitmap & (1 << v) ) {
					new_vm = v;
					_mhdebug("KEYBOARD new_vm",new_vm);
					break;
				}
			}
			if(new_vm == v) {
				select_console(0, new_vm);		
				set_leds();
			} else {
				_mhdebug("KEYBOARD ERROR bm",bm);
			}
		}
	} else
	if (ch == ADOWN) {
		/* Choose lower numbered VM */
		_mhdebug("KEYBOARD ADOWN",vm_bitmap);
		if( vmcurrent > VM0) {
			new_vm = (-1);
			for( v = vmcurrent-1; v >= VM0; v--){
				if( vm_bitmap & (1 << v) ) {
					new_vm = v;
					_mhdebug("KEYBOARD new_vm",new_vm);
					break;
	...
	if (AF1 <= ch && ch <= AF12) {
		...
		_mhdebug("TTY: kb_read (ch - AF1)", (ch - AF1));
		if( (ch - AF1) < (nr_cons)) {
			con = (ch - AF1)%nr_cons;
			select_console(con, vmcurrent);
			set_leds();
		}
	} else if( ch == AHOME ) { /*ONLY FOR TEST !!!!!! */
	   	_mhdebug("KEYBOARD AHOME", vmcurrent);
	...
	if (CF1 <= ch && ch <= CF12) {
	    switch(ch) {
  		...
  		case CF7: sigchar(&tty_table[vmcurrent].line[CONSOLE], SIGQUIT); break;
  		case CF8: sigchar(&tty_table[vmcurrent].line[CONSOLE], SIGINT); break;
  		case CF9: sigchar(&tty_table[vmcurrent].line[CONSOLE], SIGKILL); break;
  	    }
	}
  }
...
#ifdef MHYPER  
	sys_killvm(INIT_PROC_NR, SIGABRT, vmcurrent);
#else /* MHYPER */
...
/* Load a new keymap. */
  int result;
#ifdef MHYPER
  result = sys_vcopyvm(m->IO_ENDPT, rqst_vmid, (vir_bytes) m->ADDRESS,
  	SELF, VM0, (vir_bytes) keymap, 
 ...
 */
  ...
  m_ptr->m_type = result;
  sendvm(m_ptr->m_source, m_ptr, VM0);
}
...
#ifdef MHYPER
      notifyvm(proc_nr, VM0); /* FUNCTION KEYS only valid for VM0 */
#else /*  MHYPER */
      notify(proc_nr);
#endif /*  MHYPER */	  
  }
