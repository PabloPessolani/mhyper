...
typedef struct tty {
  ...
  struct tty *tty_next; 	/* link to the next tty with pending events */
...
typedef struct tty_tab_s {
		tty_t 	line[NR_CONS+NR_RS_LINES+NR_PTYS];
} tty_tab_t;

/* Memory allocated in tty.c, so extern here. */
extern tty_tab_t tty_table[NR_VMS];

extern int ccurrent;		/* currently visible console */
extern int irq_hook_id;		/* hook id for keyboard irq */
extern int nr_cons;			/* actual number of consoles */

#ifdef MHYPER
extern unsigned int vm_bitmap; /* this bitmap keeps the VMs that TTY server as a promiscuous task*/
extern int vmcurrent;		/* currently VMID of active console  */
...
_PROTOTYPE(void tty_enqueue,(tty_t *tp)					);
_PROTOTYPE(tty_t *tty_dequeue, (void)					);
...
_PROTOTYPE( void select_console, (int cons_line, int vmid)			);
...
_PROTOTYPE( void vid_mem_copy, (unsigned src, u16_t *dst,  unsigned count));

_PROTOTYPE( void print_console, (int cons, int vmid));
#endif /* (CHIP == INTEL) */
