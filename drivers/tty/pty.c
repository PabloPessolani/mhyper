...
#ifdef MHYPER
	if ((s = sys_vcopyvm(tp->tty_outproc, tp->tty_vmid, (vir_bytes) tp->tty_out_vir,
		SELF, VM0, (vir_bytes) pp->ohead, (phys_bytes) count)) != OK) {
...
#ifdef MHYPER
	if ((s = sys_vcopyvm(SELF, VM0, (vir_bytes)pp->otail,
		(vir_bytes) pp->rdproc, pp->pty_vmid, (vir_bytes) pp->rdvir, (phys_bytes) count)) != OK) {
		printf("pty tty%d: copy failed (error %d)\n",  s);
		break;
	}
#else /*  MHYPER */
...
#ifdef MHYPER
		notifyvm(pp->rdcaller, pp->pty_vmid);
#else /*  MHYPER */
		notify(pp->rdcaller);
#endif /*  MHYPER */	
  }
...
#ifdef MHYPER
	if ((s = sys_vcopyvm(pp->wrproc, pp->pty_vmid, (vir_bytes) pp->wrvir,
		SELF, VM0, (vir_bytes) &c, (phys_bytes) 1)) != OK) {
...
#ifdef MHYPER
			notifyvm(pp->wrcaller, pp->pty_vmid);
#else /*  MHYPER */
			notify(pp->wrcaller);
#endif /*  MHYPER */
	}
...
tty_t *tp;
{
  ...
  line = tp->tty_index - (NR_CONS + NR_RS_LINES);
  pp = tp->tty_priv = &pty_table[line];
  pp->tty = tp;
#ifdef MHYPER
  pp->pty_vmid = tp->tty_vmid;
#endif /*  MHYPER */  
...
#ifdef MHYPER
		notifyvm(pp->select_proc, pp->pty_vmid);
#else /*  MHYPER */
