...
#ifdef MHYPER
#define   VMDEBUG(text,value)  		if (vmid > VM0) _mhdebug(text,value);
int vmid;
#endif /*  MHYPER */
...
int main(void)
{
...
#ifdef MHYPER
	int pci_vmid;
	pci_vmid = _mhdebug("PCI: vmid=", pci_vmid);
	pci_vmid = _mhdebug("PCI: vmid=", pci_vmid);
#endif /*  MHYPER */ 
	...
	{
#ifdef MHYPER
	vmid = recvvm(ANY, &m, ANY_VM);
	if (vmid < VM0) {
		_mhdebug("PCI: receive from ANY: %d\n", vmid);
		printf("PCI: receive from ANY failed: %d\n", vmid);
		break;
	}
	if (vmid > VM0) {
	    m_ptr = &m;
		_mhdebug("PCI: vmid",vmid);
...
#define DEBUG 1	
	...
	printf("pci_init: called by '%s'\n", mp->m3_ca1);
  	_mhdebug("pci_init: m_source=", mp->m_source);
#endif
	...
	mp->m_type= 0;
#ifdef MHYPER
	r= sendvm(mp->m_source, mp, vmid);
  	_mhdebug("pci_init: sendvm=", mp->m_source);
#else /*  MHYPER */
