 ...
 */
  ...
  if ((dv = (*dp->dr_prepare)(device)) == NIL_DEV
	...
	_mhdebug("partition: device to part:",device);
	_mhdebug("partition: style:",style);
	_mhdebug("partition: atapi:",atapi);
  ...
  case P_SUB:
	...
	_mhdebug("P_SUB: disk:",disk);
	_mhdebug("P_SUB: par:",par);
	_mhdebug("P_SUB: device:",device);
  }
  ...
  for (par = 0; par < NR_PARTITIONS; par++, dv++) {
	...
	_mhdebug("partition: device:",device);
	_mhdebug("partition: par:",par);
	_mhdebug("partition: dv_size:",dv->dv_size);
