...
int main(void)
{
  ...
  void *p1, *p2, *p3;
   ...
   init_vmid = _mhdebug("INIT: main", init_vmid);
   init_vmid = _mhdebug("INIT: vmid", init_vmid);
  ...
  if ((pid = fork()) != 0) {
	...
	*rcp = "start";
  	...
  	_mhdebug("INIT: before shell", init_vmid);
  	_mhdebug(bootopts,init_vmid);
   ...
   */
	_mhdebug("INIT: entering loop", init_vmid);
