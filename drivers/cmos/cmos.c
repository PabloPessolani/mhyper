...
PUBLIC void main(void)
{
  ...
  int y2kflag;
  int result;
  int suspended[NR_VMS], vm_pendings;
  int s, i ;

 	_mhdebug("CMOS: initializing on VM",VM0);
	for(i = 0; i < NR_VMS; i++)
		suspended[i] = NONE;
	vm_pendings = 0;
...
#define CMOS_DBG 1		
		if(rqst_vmid < 0 || rqst_vmid >= NR_VMS)  {
			_mhdebug("CMOS: ERROR recvvm",rqst_vmid);
#ifdef CMOS_DBG	
		}else{
			_mhdebug("CMOS:  rqst_vmid",rqst_vmid);
#endif /* CMOS_DBG */
		}
	...
	/* Probably best to SUSPEND the caller, CMOS I/O has nasty timeouts. 
	   * This way we don't block the rest of the system. First check if
           ...
           * requests at a time. 
           */
		_mhdebug("CMOS: DEV_IOCTL IO_ENDPT",m.IO_ENDPT);

		/* check if there is a previous request pending */	
        if (vm_pendings) {
            reply(TASK_REPLY, m.m_source, m.IO_ENDPT, EBUSY);
            break;
        }
        suspended[rqst_vmid] = m.IO_ENDPT;
        reply(TASK_REPLY, m.m_source, m.IO_ENDPT, SUSPEND);

/*		_mhdebug("CMOS: DEV_IOCTL REQUEST",m.REQUEST); */
		switch(m.REQUEST) {
		...
		vm_pendings |= (1 < rqst_vmid); /* set the VM's pending bit */ 
        /* Request completed. Tell the caller to check our status. */
#ifdef MHYPER
/*		_mhdebug("CMOS: notifyvm",m.m_source); */
		notifyvm(m.m_source, rqst_vmid);
#else /* MHYPER */
		notify(m.m_source);
#endif /* MHYPER */
      ...
      case DEV_STATUS:
        ...
        /* The FS calls back to get our status. Revive the suspended 
			...
			*/
		if (suspended[rqst_vmid] == NONE)
			reply(DEV_NO_STATUS, m.m_source, NONE, OK);
        else 
            reply(DEV_REVIVE, m.m_source, suspended[rqst_vmid], result);
        suspended[rqst_vmid] = NONE;
		vm_pendings  &= ~(1 < rqst_vmid);
        break;
...
#ifdef MHYPER
	if (OK != (s=sendvm(replyee, &m, rqst_vmid)))
		_mhdebug("CMOS: ERROR sendvm",s);
#else /* MHYPER */
...
#ifdef MHYPER
  sys_vcopyvm(SELF, VM0, (vir_bytes) &time1,  who, rqst_vmid, dst_time, sizeof(struct tm));
#else /* MHYPER */
  sys_datacopy(SELF, (vir_bytes) &time1, who, dst_time, sizeof(struct tm));
#endif /* MHYPER */
